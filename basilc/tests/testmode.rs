...
fn basilc_test_mode_basic() {
    ...
    let exe_path = if let Ok(p) = env::var("CARGO_BIN_EXE_basilc") {
...
"#;
    ...
    assert!(
        stdout.contains("COMMENT: Ask for confirmation"),
        "stdout missing comment echo:\n{}",
        stdout
    );
    assert!(
        stdout.contains("COMMENT: After line input"),
        "stdout missing second comment echo:\n{}",
        stdout
    );
    assert!(
        stdout.contains("Mock input to INPUT given as"),
        "stdout missing mock INPUT log:\n{}",
        stdout
    );
    assert!(
        stdout.contains("Mock input to INPUTC$ given as"),
        "stdout missing mock INPUTC$ log:\n{}",
        stdout
    );
    assert!(
        stdout.contains("Mock input to INKEY$ given as"),
        "stdout missing mock INKEY$ log:\n{}",
        stdout
    );
    assert!(
        stdout.contains("Mock input to INKEY% given as"),
