#!/usr/bin/env bash
# Build various Basil release binaries for macOS and copy them into dist/mac
...
# - --features "obj-ai obj-csv obj-curl obj-json obj-zip obj-sqlite obj-aws obj-sql obj-orm obj-net obj-crypto" -> basilc-web, bcc-web
...
build_variant() {
  ...
  if [[ ! -x "$BASILC_BIN" ]]; then
    echo "Error: basilc binary not found at $BASILC_BIN" >&2
    exit 1
  fi
  if [[ ! -x "$BCC_BIN" ]]; then
    echo "Error: bcc binary not found at $BCC_BIN" >&2
    exit 1
  fi

  # Decide output names per label
  local basilc_out
  local bcc_out
  case "$label" in
    naked)
...
build_variant web   "obj-ai obj-csv obj-curl obj-json obj-zip obj-sqlite obj-aws obj-sql obj-orm obj-net obj-crypto"

echo "\nAll builds completed. Output directory: $DIST_DIR"