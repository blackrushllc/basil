...
pub fn register<F: FnMut(&str, TypeInfo)>(mut reg: F) {
    #[cfg(any(
        feature = "obj-orm",
        feature = "obj-orm-mysql",
        feature = "obj-orm-postgres"
    ))]
    {
        let factory = |args: &[Value]| -> Result<Rc<RefCell<dyn BasicObject>>> {
            ...
            if args.len() != 1 {
                return Err(BasilError(
                    "ORM: constructor expects 1 argument (DB object)".into(),
                ));
            }
            let db = match &args[0] {
                ...
                other => {
                    return Err(BasilError(format!(
                        "ORM: expected DB object, got {}",
                        other
                    )))
                }
            };
            let dialect = db.borrow().type_name().to_ascii_lowercase();
            if dialect != "db_postgres" && dialect != "db_mysql" {
                return Err(BasilError(
                    "ORM: unsupported DB object; use DB_POSTGRES or DB_MYSQL".into(),
                ));
            }
            Ok(Rc::new(RefCell::new(OrmObj {
                db,
                dialect: if dialect == "db_postgres" {
                    "postgres".into()
                } else {
                    "mysql".into()
                },
...
impl OrmObj {
        ...
        ObjectDescriptor {
            ...
            methods: vec![
                MethodDesc {
                    name: "Model".into(),
                    arity: 3,
                    arg_names: vec!["table$".into(), "cols$[]".into(), "pk$".into()],
                    return_type: "ORM".into(),
                },
                MethodDesc {
                    name: "ModelCsv$".into(),
                    arity: 3,
                    arg_names: vec!["table$".into(), "cols_csv$".into(), "pk$".into()],
                    return_type: "ORM".into(),
                },
                MethodDesc {
                    name: "ModelFromTable$".into(),
                    arity: 1,
                    arg_names: vec!["table$".into()],
                    return_type: "Str$".into(),
                },
                MethodDesc {
                    name: "HasMany".into(),
                    arity: 3,
                    arg_names: vec!["parent$".into(), "child$".into(), "fk$".into()],
                    return_type: "ORM".into(),
                },
                MethodDesc {
                    name: "BelongsTo".into(),
                ...
                MethodDesc {
                    ...
                    arity: 1,
                    arg_names: vec!["table$".into()],
                    return_type: "ORM_QUERY".into(),
                },
                MethodDesc {
                    name: "New".into(),
                    arity: 1,
                    arg_names: vec!["table$".into()],
                    return_type: "ORM_ROW".into(),
                },
                MethodDesc {
                    name: "Begin".into(),
                    arity: 0,
                ...
                MethodDesc {
                    name: "RowFromJson$".into(),
                    arity: 2,
                    arg_names: vec!["table$".into(), "json$".into()],
                    return_type: "ORM_ROW".into(),
                },
            ],
            examples: vec![
    ...
    fn ensure_model(&self, table: &str) -> Result<ModelMeta> {
        self.models
            .get(&table.to_string())
            .cloned()
            .ok_or_else(|| BasilError(format!("ORM.ModelNotFound: {}", table)))
    }
...
fn make_array_of_objects(
    objs: Vec<Rc<RefCell<dyn BasicObject>>>,
    type_name_hint: Option<&str>,
) -> Value {
    let elem = match type_name_hint {
        Some(t) => ElemType::Obj(Some(t.to_string())),
        None => ElemType::Obj(None),
    };
...
impl BasicObject for OrmObj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("ORM: unknown property".into()))
    }

    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("ORM: unknown or read-only property".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "MODEL" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "ORM.Model expects (table$, cols$[], pk$)".into(),
                    ));
                }
                ...
                let cols = match &args[1] {
                    ...
                    _ => {
                        return Err(BasilError(
                            "ORM.Model: cols$[] must be array of strings".into(),
                        ))
                    }
                };
            ...
            "MODELCSV$" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "ORM.ModelCsv$ expects (table$, cols_csv$, pk$)".into(),
                    ));
                }
                let table = as_str(&args[0]);
                let cols_csv = as_str(&args[1]);
                let pk = as_str(&args[2]);
                let cols: Vec<String> = cols_csv
                    .split(',')
                ...
                if cols.is_empty() {
                    return Err(BasilError(
                        "ORM.ModelCsv$: cols_csv$ must contain at least one column".into(),
                    ));
                }
                let mm = ModelMeta {
                    table: table.clone(),
            ...
            "MODELFROMTABLE$" => {
                if args.len() != 1 {
                    return Err(BasilError("ORM.ModelFromTable$ expects (table$)".into()));
                }
                let table = as_str(&args[0]);
                // fetch info schema
                let (cols_sql, pk_sql) = self.info_schema_sql(&table);
                // columns
                let cols_json = self.db.borrow_mut().call(
                    "QUERY$",
                    &[Value::Str(cols_sql.clone()), Value::Str(table.clone())],
                )?;
                ...
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    if let Value::Str(s) = cols_json {
                        // parse very simply as JSON array of objects with column_name, data_type
                        let val: serde_json::Value = serde_json::from_str(&s).map_err(|e| {
                            BasilError(format!("ORM.ModelFromTable$: parse error: {}", e))
                        })?;
                        if let Some(arr) = val.as_array() {
                            for row in arr {
                                let col = row
                                    ...
                                    .unwrap_or("");
                                let dt =
                                    row.get("data_type").and_then(|v| v.as_str()).unwrap_or("");
                                if col.is_empty() {
                                    continue;
                                }
                                let suffix = if dt.contains("int") {
                                    "%"
                ...
                // pk
                let pk_json = self.db.borrow_mut().call(
                    "QUERY$",
                    &[Value::Str(pk_sql.clone()), Value::Str(table.clone())],
                )?;
                ...
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    if let Value::Str(s) = pk_json {
                        ...
                        if let Some(arr) = val.as_array() {
                            if let Some(row0) = arr.first() {
                                if let Some(cn) =
                                    row0.get("attname").or_else(|| row0.get("column_name"))
                                {
                                    pk = format!("{}%", cn.as_str().unwrap_or(""));
                                }
                            }
                        }
                    }
                ...
                if cols.is_empty() {
                    return Err(BasilError(format!(
                        "ORM.ModelFromTable$: no columns found for {}",
                        table
                    )));
                }
                if pk.is_empty() {
            ...
            "HASMANY" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "ORM.HasMany expects (parent$, child$, fk$)".into(),
                    ));
                }
                ...
                let mut m = self
                    ...
                    .get(&parent)
                    .cloned()
                    .ok_or_else(|| BasilError(format!("ORM.ModelNotFound: {}", parent)))?;
                let pk = m.pk.clone();
                m.relations.insert(
                    child.clone(),
                    RelationMeta {
            ...
            "BELONGSTO" => {
                if args.len() != 4 {
                    return Err(BasilError(
                        "ORM.BelongsTo expects (child$, parent$, fk$, pk$)".into(),
                    ));
                }
                ...
                let mut m = self
                    ...
                    .get(&child)
                    .cloned()
                    .ok_or_else(|| BasilError(format!("ORM.ModelNotFound: {}", child)))?;
                m.relations.insert(
                    parent.clone(),
                    RelationMeta {
            ...
            "TABLE" => {
                if args.len() != 1 {
                    return Err(BasilError("ORM.Table expects (name$)".into()));
                }
                let table = as_str(&args[0]);
            ...
            "NEW" => {
                if args.len() != 1 {
                    return Err(BasilError("ORM.New expects (table$)".into()));
                }
                let table = as_str(&args[0]);
                let meta = self.ensure_model(&table)?;
            ...
            "ROWFROMJSON$" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "ORM.RowFromJson$ expects (table$, json$)".into(),
                    ));
                }
                let table = as_str(&args[0]);
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                ...
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    let v: serde_json::Value = serde_json::from_str(&json)
                        .map_err(|e| BasilError(format!("ORM.RowFromJson$: {}", e)))?;
                    let mut data = HashMap::new();
                    if let Some(obj) = v.as_object() {
                        for (k, vv) in obj.iter() {
                            data.insert(k.clone(), vv.to_string().trim_matches('"').to_string());
                        }
                    }
                ...
                {
                    return Err(BasilError(
                        "ORM.RowFromJson$: JSON disabled (enable obj-orm-*)".into(),
                    ));
                }
            }
        ...
        ObjectDescriptor {
            ...
            methods: vec![
                MethodDesc {
                    name: "Where$".into(),
                    arity: 3,
                    arg_names: vec!["col$".into(), "op$".into(), "val$".into()],
                    return_type: "ORM_QUERY".into(),
                },
                MethodDesc {
                    name: "OrderBy$".into(),
                    arity: 2,
                    arg_names: vec!["col$".into(), "dir$".into()],
                    return_type: "ORM_QUERY".into(),
                },
                MethodDesc {
                    name: "Limit%".into(),
                ...
                MethodDesc {
                    name: "Pluck".into(),
                    arity: 2,
                    arg_names: vec!["value_col$".into(), "key_col$".into()],
                    return_type: "ARRAY|DICT".into(),
                },
                MethodDesc {
                    name: "Get".into(),
                    arity: 0,
                    arg_names: vec![],
                    return_type: "ARRAY<ORM_ROW>".into(),
                },
                MethodDesc {
                    name: "Find%".into(),
                    arity: 1,
                    arg_names: vec!["pk%".into()],
                    return_type: "ORM_ROW".into(),
                },
                MethodDesc {
                    name: "First".into(),
            ...
            examples: vec![
                "DIM q@ = orm@.Table(\"users\").Where$(\"email$\", \"=\", \"a@example.com\")"
                    .into(),
            ],
        }
    }
    ...
    fn compile_select(&self) -> (String, Vec<String>) {
        ...
        if !self.wheres.is_empty() {
            ...
            for (i, (col, op, val)) in self.wheres.iter().enumerate() {
                ...
                sql.push_str(&format!(
                    "{} {} {}",
                    self.quote_ident(col.trim_end_matches(['%', '$'])),
                    op,
                    ph
                ));
...
impl BasicObject for QueryObj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("ORM_QUERY: unknown property".into()))
    }

    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError(
            "ORM_QUERY: unknown or read-only property".into(),
        ))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "WHERE$" => {
                if args.len() != 3 {
                    return Err(BasilError("Where$(col$,op$,val$)".into()));
                }
                self.wheres
                    .push((as_str(&args[0]), as_str(&args[1]), as_str(&args[2])));
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            }
            "ORDERBY$" => {
                if args.len() != 2 {
                    return Err(BasilError("OrderBy$(col$,dir$)".into()));
                }
                self.order = Some((as_str(&args[0]), as_str(&args[1])));
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            }
            "LIMIT%" => {
                if args.len() != 1 {
                    return Err(BasilError("Limit%(n%)".into()));
                }
                let n = match &args[0] {
                    Value::Int(i) => *i,
                    Value::Num(n) => n.trunc() as i64,
                    other => {
                        return Err(BasilError(format!("Limit% expects int, got {}", other)));
                    }
                };
                self.limit = Some(n);
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            ...
            "OFFSET%" => {
                if args.len() != 1 {
                    return Err(BasilError("Offset%(n%)".into()));
                }
                let n = match &args[0] {
                    Value::Int(i) => *i,
                    Value::Num(n) => n.trunc() as i64,
                    other => {
                        return Err(BasilError(format!("Offset% expects int, got {}", other)));
                    }
                };
                self.offset = Some(n);
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            ...
            "WITH$" => {
                if args.len() != 1 {
                    return Err(BasilError("With$(relation$)".into()));
                }
                self.with.push(as_str(&args[0]));
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            }
            "SELECT$" => {
                if args.len() != 1 {
                    return Err(BasilError("Select$(cols): expects 1 argument".into()));
                }
                ...
                let cols: Vec<String> = match &args[0] {
                    ...
                    other => {
                        return Err(BasilError(format!(
                            "Select$ expects array, list, or string; got {}",
                            other
                        )));
                    }
                };
            ...
            "UNIQUE" => {
                if !args.is_empty() {
                    return Err(BasilError("Unique() expects 0 arguments".into()));
                }
                self.distinct = true;
                Ok(Value::Object(Rc::new(RefCell::new(self.clone()))))
            }
            "PLUCK" => {
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(BasilError("Pluck(value_col$ [, key_col$])".into()));
                }
                let value_name = as_str(&args[0]);
                let key_name_opt = if args.len() == 2 {
                    Some(as_str(&args[1]))
                } else {
                    None
                };
                ...
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    ...
                    let arr: serde_json::Value = serde_json::from_str(&json)
                        .map_err(|e| BasilError(format!("ORM.Query.Pluck JSON parse: {}", e)))?;
                    ...
                    let infer_type = |col: &str| -> ElemType {
                        if col.ends_with('%') {
                            ...
                            if self
                                ...
                                .cols
                                .iter()
                                .any(|c| c.eq_ignore_ascii_case(&format!("{base}%")))
                            {
                                ElemType::Int
                            } else if self
                                .meta
                    ...
                    if let Some(key_name) = key_name_opt {
                        ...
                        for (i, obj) in rows.iter().enumerate() {
                            ...
                            let v = match vty {
                                ElemType::Int => {
                                    sval.parse::<i64>().map(Value::Int).map_err(|_| {
                                        BasilError(format!(
                                            "Pluck: value at row {} is not an integer: {}",
                                            i + 1,
                                            sval
                                        ))
                                    })?
                                ...
                                ElemType::Num => {
                                    sval.parse::<f64>().map(Value::Num).map_err(|_| {
                                        BasilError(format!(
                                            "Pluck: value at row {} is not a number: {}",
                                            i + 1,
                                            sval
                                        ))
                                    })?
                        ...
                        let elem = match vty {
                            ...
                            ElemType::Num => ElemType::Num,
                            _ => ElemType::Str,
                        };
                        return Ok(Value::Array(Rc::new(ArrayObj {
                            elem,
                            dims: vec![data.len()],
                            data: RefCell::new(data),
                        })));
                ...
                {
                    let _ = res;
                    return Err(BasilError("ORM.Query.Pluck requires SQL features".into()));
                }
            }
            "GET" => {
                let (sql, mut params) = self.compile_select();
                ...
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    if let Value::Str(json) = res {
                        let arr: serde_json::Value = serde_json::from_str(&json)
                            .map_err(|e| BasilError(format!("ORM.Query.Get JSON parse: {}", e)))?;
                            ...
                            for obj in v {
                                ...
                                if let Some(map) = obj.as_object() {
                                    for (k, vv) in map.iter() {
                                        data.insert(
                                            k.clone(),
                                            vv.to_string().trim_matches('"').to_string(),
                                        );
                                    }
                                }
                                let row = RowObj::new(
            ...
            "FIND%" => {
                if args.len() != 1 {
                    return Err(BasilError("Find%(pk%)".into()));
                }
                ...
                let mut call_args: Vec<Value> = vec![Value::Str(sql)];
                for p in params {
                    call_args.push(Value::Str(p));
                }
                let res = self.db.borrow_mut().call("QUERY$", &call_args)?;
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
                {
                    if let Value::Str(json) = res {
                        let arr: serde_json::Value = serde_json::from_str(&json).map_err(|e| {
                            BasilError(format!("ORM.Query.Find% JSON parse: {}", e))
                        })?;
                        if let Some(v) = arr.as_array() {
            ...
            "FIRST" => {
                ...
                match arr {
                    Value::Array(rc) => {
                        if rc.data.borrow().is_empty() {
                            Ok(Value::Null)
                        } else {
                            Ok(rc.data.borrow()[0].clone())
                        }
                    }
                    _ => Ok(Value::Null),
                }
...
impl RowObj {
    fn new(
        db: Rc<RefCell<dyn BasicObject>>,
        dialect: String,
        meta: ModelMeta,
        init: Option<HashMap<String, String>>,
    ) -> Self {
        let mut data = HashMap::new();
        if let Some(m) = &init {
            data.extend(m.clone());
    ...
    fn pk_plain(&self) -> String {
        self.meta.pk.trim_end_matches(['%', '$']).to_string()
    }
        ...
        match name.to_ascii_uppercase().as_str() {
            ...
            _ => {
                ...
                let col = self
                    ...
                    .cloned()
                    .ok_or_else(|| {
                        BasilError(format!("ORM.UnknownColumn: {}.{}", self.meta.table, name))
                    })?;
        ...
        match method.to_ascii_uppercase().as_str() {
            "SAVE" => {
                ...
                if !has_pk {
                    ...
                    if cols.is_empty() {
                        return Err(BasilError(format!(
                            "ORM.ValidationFailed: {} (<field> required)",
                            self.meta.table
                        )));
                    }
                    ...
                    sql.push_str(&format!(
                        "INSERT INTO {} ({} ) VALUES (",
                        self.quote_ident(&self.meta.table),
                        cols.iter()
                    ...
                    for i in 0..vals.len() {
                        ...
                        params.push(Value::Str(vals[i].clone()));
                    }
                    sql.push_str(")");
                    let mut call_args = vec![Value::Str(sql)];
                    ...
                    for (col, is_dirty) in self.dirty.clone().into_iter() {
                        if is_dirty {
                            sets.push(format!(
                                ...
                                self.placeholder(sets.len() + 1)
                            ));
                            params
                                .push(Value::Str(self.data.get(&col).cloned().unwrap_or_default()));
                        }
                    }
                    if sets.is_empty() {
                    ...
                    let sql = format!(
                        ...
                        self.quote_ident(&self.meta.table),
                        sets.join(", "),
                        self.quote_ident(&self.pk_plain()),
                        self.placeholder(sets.len() + 1)
                    );
                    params.push(Value::Str(
                        self.data.get(&pkcol).cloned().unwrap_or_default(),
                    ));
                    let mut call_args = vec![Value::Str(sql)];
                    call_args.extend(params);
                    let _ = self.db.borrow_mut().call("EXECUTE", &call_args)?;
            ...
            "DELETE" => {
                ...
                if v.is_empty() {
                    return Err(BasilError(format!(
                        "ORM.ValidationFailed: {} ({} required)",
                        self.meta.table, self.meta.pk
                    )));
                }
                let sql = if self.dialect == "postgres" {
                    format!(
                        "DELETE FROM {} WHERE {} = $1",
                        self.quote_ident(&self.meta.table),
                        self.quote_ident(&pkcol)
                ...
                let _ = self
                    .db
                    .borrow_mut()
                    .call("EXECUTE", &[Value::Str(sql), Value::Str(v)])?;
                Ok(Value::Int(1))
            }
            "TOJSON$" => {
                #[cfg(any(feature = "obj-orm-mysql", feature = "obj-orm-postgres"))]
            ...
            _other => {
                ...
                if method.ends_with("()") || !args.is_empty() {
                    return Err(BasilError(format!(
                        "Unknown method '{}' on ORM_ROW",
                        method
                    )));
                }
                ...
                if let Some(rel) = self.meta.relations.get(&name.to_string()) {
                    match rel.kind.as_str() {
                        "has_many" => {
                            ...
                            let (sql, params) = if self.dialect == "postgres" {
                                (
                                    format!("SELECT * FROM {} WHERE {} = $1", rel.other, fk),
                                    vec![pkv],
                                )
                            } else {
                                (
                                    format!("SELECT * FROM {} WHERE {} = ?", rel.other, fk),
                                    vec![pkv],
                                )
                            };
                            let mut call_args: Vec<Value> = vec![Value::Str(sql)];
                        ...
                        "belongs_to" => {
                            ...
                            let (sql, params) = if self.dialect == "postgres" {
                                ...
                                (
                                    format!("SELECT * FROM {} WHERE {} = ? LIMIT 1", rel.other, pk),
                                    vec![fkv],
                                )
                            };
                            let mut call_args: Vec<Value> = vec![Value::Str(sql)];
