...
impl BasilicaApp {
    pub fn new(cfg: BasilicaConfig) -> Self {
        Self {
            config: cfg,
            ..Default::default()
        }
    ...
    pub fn ui(&mut self, ctx: &Context) {
        ...
        egui::TopBottomPanel::top("top").show(ctx, |ui| {
            egui::menu::bar(ui, |ui| {
                ...
                if ui.button("Run Script…").clicked() {
                    self.run_dialog = Some(RunDialogState {
                        path: None,
                        is_gui: false,
                        mode: MenuMode::Run,
                    });
                }
                if ui.button("Manage Scripts…").clicked() {
        ...
        egui::SidePanel::left("busy").show(ctx, |ui| {
            ...
            if ui.button("Open File…").clicked() {
                if let Some(path) = rfd::FileDialog::new().pick_file() {
                    self.alerts.push(format!("Picked {}", path.display()));
                }
            }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            for (i, c) in self.consoles.iter_mut().enumerate() {
                egui::CollapsingHeader::new(format!("{}", c.title))
                    .default_open(true)
                    .show(ui, |ui| {
                        egui::ScrollArea::vertical()
                            .id_salt(format!("log-{}", i))
                            .show(ui, |ui| {
        ...
        if let Some(state) = &mut self.run_dialog {
                ...
                .show(ctx, |ui| {
                    if ui.button("Choose file…").clicked() {
                        if let Some(p) = rfd::FileDialog::new()
                            .add_filter("Basil", &["basil"])
                            .pick_file()
                        {
                            state.path = Some(p.to_string_lossy().to_string());
                        }
                    }
            ...
            if let Some((item, is_gui)) = to_launch {
                self.launch_item(item, is_gui);
    ...
    fn manage_scripts(&mut self, ctx: &Context) {
        egui::Window::new("Manage Scripts")
            .open(&mut self.manage_open)
            .show(ctx, |ui| {
                egui::widgets::global_theme_preference_buttons(ui);
                ui.horizontal(|ui| {
                    if ui.button("Add CLI").clicked() {
                        self.config.cli_scripts.push(blank_item());
                    }
                    if ui.button("Add GUI").clicked() {
                        self.config.gui_scripts.push(blank_item());
                    }
                    if ui.button("Save").clicked() {
                        let _ = config::save_atomic(&self.config);
                    }
                });
                ui.separator();
        ...
        for (i, it) in items.iter_mut().enumerate() {
            ...
            if it.kind == MenuKind::File {
                ...
                if ui.text_edit_singleline(&mut p).changed() {
                    it.path = if p.is_empty() { None } else { Some(p) };
                }
                if ui.button("…").clicked() {
                    if let Some(sel) = rfd::FileDialog::new()
                        .add_filter("Basil", &["basil"])
                        .pick_file()
                    {
                        it.path = Some(sel.to_string_lossy().to_string());
                    }
                }
            } else {
                ui.label("");
            ...
            let mut a = it.args.clone().unwrap_or_default();
            if ui.text_edit_singleline(&mut a).changed() {
                it.args = if a.is_empty() { None } else { Some(a) };
            }
