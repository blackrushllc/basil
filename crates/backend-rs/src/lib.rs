...
fn render_main_rs(src_path: &Path, module: &Module) -> String {
    ...
    render_instrs(&module.main.body, &mutated, &mut vars, &mut body, 0);
...
fn collect_assign_counts(instrs: &Vec<Instr>, counts: &mut HashMap<String, usize>) {
    for ins in instrs {
        match ins {
            Instr::Assign { var, .. } => {
                let e = counts.entry(var.clone()).or_insert(0);
                *e += 1;
            }
...
fn render_instrs(instrs: &Vec<Instr>, mutated: &HashSet<String>, vars: &mut HashMap<String, String>, out: &mut String, mut indent: usize) {
    ...
    for ins in instrs {
        match ins {
            ...
            Instr::For { var, start, end, step, body } => {
                ...
                indent += 1;
                // extend var map
                let prev = vars.insert(var.clone(), rust_var.clone());
                render_instrs(body, mutated, vars, out, indent);
                // restore
                if let Some(prev_name) = prev { vars.insert(var.clone(), prev_name); } else { vars.remove(var); }
            ...
            Instr::Assign { var, expr } => {
                ...
                if vars.contains_key(var) {
                    ...
                    vars.insert(var.clone(), rust_var.clone());
                    let mut_kw = if mutated.contains(var) { "mut " } else { "" };
            ...
            Instr::If { cond, then_body, else_body } => {
                ...
                indent += 1;
                render_instrs(then_body, mutated, vars, out, indent);
                indent -= 1;
                if !else_body.is_empty() {
                    out.push_str(&format!("{}}} else {{\n", ind(indent)));
                    indent += 1;
                    render_instrs(else_body, mutated, vars, out, indent);
                    indent -= 1;
                }
                out.push_str(&format!("{}}}\n", ind(indent)));
            }
...
fn render_expr_as_string(e: &Expr, vars: &HashMap<String, String>) -> String {
    match e {
        ...
        Expr::Var(name) => {
            let v = vars.get(name).cloned().unwrap_or_else(|| sanitize_var(name));
            format!("{}.to_string()", v)
        }
        Expr::Add(a, b) => {
...
fn render_expr_as_i64(e: &Expr, vars: &HashMap<String, String>) -> String {
    match e {
        Expr::Int(i) => format!("{}", i),
        Expr::Var(name) => vars.get(name).cloned().unwrap_or_else(|| sanitize_var(name)),
