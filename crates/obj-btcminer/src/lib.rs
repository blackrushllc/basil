            ...
            factory: |args| {
                if args.len() < 4 {
                    return Err(BasilError(
                        "BTC_ASIC expects 4 args (kind$, host$, user$, pass$)".into(),
                    ));
                }
                ...
                let profile = if kind == "generic-http-json" {
                    let prof_json =
                        include_str!("../../btcminer-core/profiles/antminer_http_v1.json");
                    serde_json::from_str::<ProfileConfig>(prof_json)
    ...
    add_type(
        ...
        TypeInfo {
            ...
            descriptor: || ObjectDescriptor {
                ...
                methods: vec![
                    MethodDesc {
                        name: "RULE_TEMP_MAX%".into(),
                        arity: 1,
                        arg_names: vec!["celsius#".into()],
                        return_type: "Integer".into(),
                    },
                    MethodDesc {
                        name: "RULE_HASHRATE_MIN%".into(),
                        arity: 1,
                        arg_names: vec!["ghs#".into()],
                        return_type: "Integer".into(),
                    },
                    MethodDesc {
                        name: "RULE_REJECT_RATE_MAX%".into(),
                        arity: 1,
                        arg_names: vec!["pct#".into()],
                        return_type: "Integer".into(),
                    },
                    MethodDesc {
                        name: "EVALUATE@".into(),
                        arity: 1,
                        arg_names: vec!["snapshot@".into()],
                        return_type: "List".into(),
                    },
                ],
                examples: vec![],
...
impl BasicObject for BtcMinerMgr {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("No properties on BTC_MINER_MGR".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            other => Err(BasilError(format!(
                "Unknown method '{}' on BTC_MINER_MGR",
                other
            ))),
        }
    }
...
impl BasicObject for BtcAsic {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("No properties on BTC_ASIC".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("No properties on BTC_ASIC".into()))
    }
    fn call(&mut self, method: &str, _args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            "STATS@" => match self.adapter.stats() {
                Ok(stats) => {
            ...
            "REBOOT%" => match self.adapter.reboot() {
                Ok(_) => Ok(Value::Int(1)),
                Err(e) => {
...
impl BasicObject for BtcAlerts {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("No properties on BTC_ALERTS".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("No properties on BTC_ALERTS".into()))
    }
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "RULE_TEMP_MAX%" => {
                self.eval.rules.temp_max = args.get(0).and_then(|v| match v {
                    Value::Num(n) => Some(*n),
            ...
            "RULE_HASHRATE_MIN%" => {
                self.eval.rules.hashrate_min = args.get(0).and_then(|v| match v {
                    Value::Num(n) => Some(*n),
                    Value::Int(i) => Some(*i as f64),
                    _ => None,
                });
            ...
            "RULE_REJECT_RATE_MAX%" => {
                self.eval.rules.reject_rate_max = args.get(0).and_then(|v| match v {
                    Value::Num(n) => Some(*n),
                    Value::Int(i) => Some(*i as f64),
                    _ => None,
                });
            ...
            "EVALUATE@" => {
                let snap_val = args
                    .get(0)
                    .ok_or_else(|| BasilError("EVALUATE@: snapshot required".into()))?;
                let jv = basil_to_json(snap_val);
                let snap: MinerSnapshot = serde_json::from_value(jv)
                    .map_err(|e| BasilError(format!("Invalid snapshot: {}", e)))?;
                let alerts = self.eval.evaluate(&snap);
                let list = alerts.into_iter().map(Value::Str).collect();
                Ok(Value::List(Rc::new(RefCell::new(list))))
            }
            other => Err(BasilError(format!(
                "Unknown method '{}' on BTC_ALERTS",
                other
            ))),
        }
    }
...
impl BasicObject for BtcLogSqlite {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("No properties on BTC_LOG_SQLITE".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("No properties on BTC_LOG_SQLITE".into()))
    }
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            "WRITE_SNAPSHOT%" => {
                let snap_val = args
                    .get(0)
                    .ok_or_else(|| BasilError("WRITE_SNAPSHOT%: snapshot required".into()))?;
                let jv = basil_to_json(snap_val);
                let snap: MinerSnapshot = serde_json::from_value(jv)
                    .map_err(|e| BasilError(format!("Invalid snapshot: {}", e)))?;
            ...
            "LAST_ERR$" => Ok(Value::Str(self.last_err.clone().unwrap_or_default())),
            other => Err(BasilError(format!(
                "Unknown method '{}' on BTC_LOG_SQLITE",
                other
            ))),
        }
    }
