...
pub async fn run_script(app: &AppState, req: Request<Body>, path: PathBuf) -> Result<Response> {
    ...
    let _co = compile::compile_if_stale(
        &path,
        &bytecode_path,
        app.cfg.bytecode_dir.as_ref().map(|b| b.as_std_path()),
    )
    .await?;
    ...
    if let Some(mut sin) = child.stdin.take() {
        let body_bytes = to_bytes(Body::from(body), app.cfg.upload_limit)
            .await
            .unwrap_or_default();
    ...
    *resp.headers_mut() = so.headers;
    if let Some(status) = so.status {
        *resp.status_mut() = status;
    } else if resp.headers().get(header::LOCATION).is_some() {
