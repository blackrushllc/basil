...
pub fn set_thread_context(
    ...
    pending_menu: Option<Arc<Mutex<PendingConfig>>>,
) {
...
pub fn register_hosts(reg: &mut Registry) {
    ...
    reg.register(
        ...
        TypeInfo {
            ...
            descriptor: menu_desc,
            constants: menu_consts,
        },
    );
}
...
fn app_desc() -> ObjectDescriptor {
    ObjectDescriptor {
        type_name: "APP".into(),
        version: "1.0".into(),
        summary: "Basilica host app".into(),
        properties: vec![],
        methods: vec![
            MethodDesc {
                name: "OPEN_FILE$".into(),
...
fn app_consts() -> Vec<(String, Value)> {
    vec![]
}
...
impl BasicObject for AppObj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("Unknown property".into()))
    }
        ...
        match method.to_ascii_uppercase().as_str() {
            ...
            "ALERT%" => {
                ...
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::AppAlert(msg));
                }
                Ok(Value::Int(1))
            }
            "START_ANIM%" => {
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::AppStartAnim);
                }
                Ok(Value::Int(1))
            }
            "STOP_ANIM%" => {
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::AppStopAnim);
                }
                Ok(Value::Int(1))
            }
...
fn web_desc() -> ObjectDescriptor {
    ObjectDescriptor {
        type_name: "WEB".into(),
        version: "1.0".into(),
        summary: "Basilica webview".into(),
        properties: vec![],
        methods: vec![
            MethodDesc {
                name: "SET_HTML$".into(),
                arity: 1,
                arg_names: vec!["html$".into()],
                return_type: "STRING".into(),
            },
            MethodDesc {
                name: "EVAL$".into(),
                arity: 1,
                arg_names: vec!["js$".into()],
                return_type: "STRING".into(),
            },
            MethodDesc {
                name: "ON%".into(),
                arity: 3,
                arg_names: vec!["event$".into(), "id$".into(), "label".into()],
                return_type: "INT".into(),
            },
        ],
        examples: vec![],
...
fn web_consts() -> Vec<(String, Value)> {
    vec![]
}
...
impl BasicObject for WebObj {
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "SET_HTML$" => {
                let html = to_s(args.get(0));
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::WebSetHtml(html));
                }
                Ok(Value::Str(String::new()))
            }
            "EVAL$" => {
                let js = to_s(args.get(0));
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::WebEval(js));
                }
                Ok(Value::Str(String::new()))
            }
            "ON%" => {
                let event = to_s(args.get(0));
                let id = to_s(args.get(1));
                let label = to_s(args.get(2));
                if let Some(tx) = &self.tx {
                    let _ = tx.send(HostRequest::WebOn { event, id, label });
                }
                Ok(Value::Int(1))
            }
...
struct MenuObj {
    pending: Option<Arc<Mutex<PendingConfig>>>,
}
...
fn menu_desc() -> ObjectDescriptor {
    ObjectDescriptor {
        type_name: "BASILICA.MENU".into(),
        version: "1.0".into(),
        summary: "Basilica bootstrap menu editor".into(),
        properties: vec![],
        methods: vec![
            MethodDesc {
                name: "CLEAR%".into(),
...
fn menu_consts() -> Vec<(String, Value)> {
    vec![]
}
...
impl BasicObject for MenuObj {
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        let Some(p) = &self.pending else {
            return Err(BasilError("BASILICA.MENU not enabled in this run".into()));
        };
        match method.to_ascii_uppercase().as_str() {
            "CLEAR%" => {
                let mut m = p.lock();
            ...
            "ADD_CLI%" => {
                add_item(p, true, args);
                Ok(Value::Int(1))
            }
            "ADD_GUI%" => {
                add_item(p, false, args);
                Ok(Value::Int(1))
            }
            "SAVE%" => {
                let mut m = p.lock();
...
fn add_item(pending: &Arc<Mutex<PendingConfig>>, cli: bool, args: &[Value]) {
    ...
    let item = MenuItem {
        ...
        mode,
        kind: kind.clone(),
        path: if kind.eq_ignore_ascii_case("file") && !path.is_empty() {
            Some(path)
        } else {
            None
        },
        args: if argz.is_empty() { None } else { Some(argz) },
    };
    if cli {
        p.cli_scripts.push(item);
    } else {
        p.gui_scripts.push(item);
    }
}
