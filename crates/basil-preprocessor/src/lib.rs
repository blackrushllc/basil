...
pub fn preprocess<'a>(text: &str, opts: &PreprocessOptions<'a>) -> Result<PreprocessResult, PreprocessError> {
    ...
    let root_dir = if opts.root_path.is_file() { opts.root_path.parent().unwrap_or(Path::new(".")).to_path_buf() } else { opts.root_path.clone() };
...
fn expand<'a>(
    ...
    for (lineno0, raw_line) in text.lines().enumerate() {
        ...
        while i < line_bytes.len() && line_bytes[i].is_ascii_whitespace() { i += 1; }
        let directive = if i < line_bytes.len() && line_bytes[i] == b'#' { Some(&raw_line[i..]) } else { None };
        if let Some(rest) = directive {
            ...
            if let Some(rem) = rest_trim.strip_prefix("if") {
                ...
                let val = if parent_active { eval_expr(expr, macros, current_file, lineno, opts)? } else { false };
                cond_stack.push(CondFrame { parent_active, active: parent_active && val, matched: val, saw_else: false });
                continue;
            } else if let Some(rem) = rest_trim.strip_prefix("elif") {
                let expr = rem.trim();
                let Some(frame) = cond_stack.last_mut() else { return Err(PreprocessError::Message(format!("#elif without matching #if at line {}", lineno))); };
                if frame.saw_else { return Err(PreprocessError::Message(format!("#elif after #else at line {}", lineno))); }
                let val = if frame.parent_active && !frame.matched { eval_expr(expr, macros, current_file, lineno, opts)? } else { false };
                frame.active = frame.parent_active && !frame.matched && val;
                if val { frame.matched = true; }
                continue;
            } else if rest_trim == "else" {
                let Some(frame) = cond_stack.last_mut() else { return Err(PreprocessError::Message(format!("#else without matching #if at line {}", lineno))); };
                if frame.saw_else { return Err(PreprocessError::Message(format!("duplicate #else at line {}", lineno))); }
                frame.saw_else = true;
                frame.active = frame.parent_active && !frame.matched;
                continue;
            } else if rest_trim == "endif" {
                if cond_stack.pop().is_none() { return Err(PreprocessError::Message(format!("#endif without matching #if at line {}", lineno))); }
                continue;
            } else if let Some(rem) = rest_trim.strip_prefix("define") {
                let active = cond_stack.iter().all(|f| f.active);
                if active {
                    let def = rem.trim();
                    if def.is_empty() { return Err(PreprocessError::Message(format!("invalid #define at line {}", lineno))); }
                    // NAME [value]
                    let mut parts = def.splitn(2, char::is_whitespace);
                    let name = parts.next().unwrap().to_string();
                    let val = parts.next().unwrap_or("").trim();
                    if is_builtin(&name) { return Err(PreprocessError::Message(format!("cannot redefine built-in macro {} at line {}", name, lineno))); }
                    if val.is_empty() {
                ...
                if active {
                    let name = rem.trim();
                    if is_builtin(name) { return Err(PreprocessError::Message(format!("cannot undef built-in macro {} at line {}", name, lineno))); }
                    macros.remove(name);
                }
                continue;
                ...
                let (form, logical) = parse_include_arg(arg).map_err(|m| PreprocessError::Message(format!("{} at line {}", m, lineno)))?;
        ...
        if active {
            ...
            if *size_left < need { return Err(PreprocessError::Message("preprocessed output exceeds size limit".into())); }
    ...
    if depth == 0 && !cond_stack.is_empty() {
        return Err(PreprocessError::Message("unterminated #if (missing #endif)".into()));
    }
    Ok(())
}
...
fn parse_include_arg(arg: &str) -> Result<(IncludeForm, String), String> {
    let arg = arg.trim();
    if arg.is_empty() { return Err("missing include path".into()); }
    let bytes = arg.as_bytes();
    if bytes[0] == b'"' {
        if let Some(end) = arg[1..].find('"') { return Ok((IncludeForm::Quoted, arg[1..1+end].to_string())); }
        return Err("unterminated string in #include".into());
    }
    if bytes[0] == b'<' {
        if let Some(end) = arg[1..].find('>') { return Ok((IncludeForm::Angle, arg[1..1+end].to_string())); }
        return Err("unterminated angle include in #include".into());
    }
    // bare: stop at first whitespace
    let mut end = arg.len();
    for (i, ch) in arg.char_indices() {
        if ch.is_whitespace() { end = i; break; }
    }
    let token = &arg[..end];
    if token.contains(' ') { return Err("spaces in include path require quotes".into()); }
...
fn resolve_in_search_paths(logical: &str, cur_dir: &Path, opts: &PreprocessOptions) -> Option<(PathBuf, String)> {
    ...
    // 2) project root
    let proj_root = if opts.root_path.is_file() { opts.root_path.parent().unwrap_or(Path::new(".")) } else { opts.root_path.as_path() };
    candidates.push(proj_root.join(logical_path));
    // 3) -I
    for dir in &opts.include_paths { candidates.push(dir.join(logical_path)); }
    // 4) env paths
    for dir in &opts.env_paths { candidates.push(dir.join(logical_path)); }

    for cand in candidates {
        if let Ok(abs) = fs::canonicalize(&cand) {
            if abs.is_file() {
                if let Ok(s) = fs::read_to_string(&abs) { return Some((abs, s)); }
            }
        }
    }
    None
...
fn resolve_any(logical: &str, cur_dir: &Path, opts: &PreprocessOptions, allow_embedded_fallback: bool) -> Option<Resolved> {
...
fn is_builtin(name: &str) -> bool {
    matches!(name, "__version__" | "__file__" | "__line__" | "__os__" | "__engine__" | "__debug__")
}

#[derive(Clone, Debug)]
enum Val { Int(i64), Str(String), Bool(bool) }

impl Val {
    fn truthy(&self) -> bool {
        match self { Val::Int(i) => *i != 0, Val::Str(s) => !s.is_empty(), Val::Bool(b) => *b }
    }
}

fn eval_expr(expr: &str, macros: &HashMap<String, MacroValue>, current_file: &str, line: usize, opts: &PreprocessOptions) -> Result<bool, PreprocessError> {
...
fn resolve_ident(name: &str, macros: &HashMap<String, MacroValue>, current_file: &str, line: usize, opts: &PreprocessOptions) -> Option<Val> {
    if name == "defined" { return None; } // handled specially as function
    match name {
        "__file__" => Some(Val::Str(current_file.to_string())),
        "__line__" => Some(Val::Int(line as i64)),
        "__engine__" => Some(Val::Str(opts.engine_name.clone().unwrap_or_else(|| "basilc".to_string()))),
        "__version__" => Some(Val::Str(opts.version.clone().unwrap_or_else(|| "0".to_string()))),
        "__debug__" => Some(Val::Int(0)),
        "__os__" => Some(Val::Str(current_os_string())),
        _ => macros.get(name).map(|m| match m { MacroValue::Bool(b)=>Val::Bool(*b), MacroValue::Int(i)=>Val::Int(*i), MacroValue::Str(s)=>Val::Str(s.clone()) }),
    }
}
...
struct Parser<'a> { s: &'a str, i: usize }

impl<'a> Parser<'a> {
    fn new(s: &'a str) -> Self { Self { s, i: 0 } }
    fn peek(&self) -> Option<char> { self.s[self.i..].chars().next() }
    fn bump(&mut self) -> Option<char> { if let Some(c) = self.peek() { self.i += c.len_utf8(); Some(c) } else { None } }
    fn eat_ws(&mut self) { while let Some(c) = self.peek() { if c.is_whitespace() { self.bump(); } else { break; } } }

    fn parse_expr<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        self.eat_ws();
    ...
    fn parse_or<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        let mut left = self.parse_and(resolve)?;
        loop {
            self.eat_ws();
            if self.match_str("||") { let right = self.parse_and(resolve)?; left = Val::Bool(left.truthy() || right.truthy()); } else { break; }
        }
        Ok(left)
    }
    fn parse_and<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        let mut left = self.parse_cmp(resolve)?;
        loop {
            self.eat_ws();
            if self.match_str("&&") { let right = self.parse_cmp(resolve)?; left = Val::Bool(left.truthy() && right.truthy()); } else { break; }
        }
        Ok(left)
    }
    fn parse_cmp<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        let mut left = self.parse_unary(resolve)?;
        loop {
            self.eat_ws();
            if let Some(op) = self.match_any(&["==","!=","<=",">=","<",">"]) {
                let right = self.parse_unary(resolve)?;
                left = Val::Bool(compare(&left, op, &right).ok_or_else(|| PreprocessError::Message(format!("invalid comparison types for '{}'", op)))?);
            } else { break; }
        }
        Ok(left)
    }
    fn parse_unary<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        self.eat_ws();
        if self.match_char('!') { let v = self.parse_unary(resolve)?; return Ok(Val::Bool(!v.truthy())); }
        self.parse_primary(resolve)
    }
    fn parse_primary<F: Fn(&str)->Option<Val>>(&mut self, resolve: &F) -> Result<Val, PreprocessError> {
        self.eat_ws();
        if self.match_char('(') { let v = self.parse_expr(resolve)?; self.eat_ws(); if !self.match_char(')') { return Err(PreprocessError::Message("expected ')'".into())); } return Ok(v); }
        if self.peek() == Some('"') { return Ok(Val::Str(self.parse_string()?)); }
        if let Some(d) = self.peek() { if d.is_ascii_digit() || d == '-' { return Ok(Val::Int(self.parse_int()?)); } }
        // identifier or defined(NAME)
        let ident = self.parse_ident()?;
        if ident == "defined" {
            self.eat_ws();
            if !self.match_char('(') { return Err(PreprocessError::Message("expected '(' after defined".into())); }
            let name = self.parse_ident()?;
            self.eat_ws();
            if !self.match_char(')') { return Err(PreprocessError::Message("expected ')' after defined(NAME)".into())); }
            return Ok(Val::Bool(resolve(&name).is_some()));
        }
        if let Some(v) = resolve(&ident) { Ok(v) } else { Ok(Val::Bool(false)) }
    }
    ...
    fn parse_ident(&mut self) -> Result<String, PreprocessError> {
        ...
        if s.is_empty() { Err(PreprocessError::Message("expected identifier".into())) } else { Ok(s) }
    }
    fn parse_string(&mut self) -> Result<String, PreprocessError> {
        if !self.match_char('"') { return Err(PreprocessError::Message("expected string".into())); }
        let mut s = String::new();
        while let Some(c) = self.bump() {
            if c == '"' { return Ok(s); }
            if c == '\\' {
                if let Some(n) = self.bump() { s.push(n); } else { break; }
            } else { s.push(c); }
        }
        Err(PreprocessError::Message("unterminated string literal".into()))
    }
    fn parse_int(&mut self) -> Result<i64, PreprocessError> {
        let mut s = String::new();
        if self.peek() == Some('-') { s.push('-'); self.bump(); }
        while let Some(c) = self.peek() { if c.is_ascii_digit() { s.push(c); self.bump(); } else { break; } }
        s.parse::<i64>().map_err(|_| PreprocessError::Message("invalid integer".into()))
    }
    fn match_char(&mut self, ch: char) -> bool { if self.peek() == Some(ch) { self.bump(); true } else { false } }
    fn match_str(&mut self, s: &str) -> bool {
        if self.s[self.i..].starts_with(s) { self.i += s.len(); true } else { false }
    }
    fn match_any<'b>(&mut self, ops: &'b [&str]) -> Option<&'b str> {
        for &op in ops { if self.s[self.i..].starts_with(op) { self.i += op.len(); return Some(op); } }
        None
    }
}
...
fn compare(a: &Val, op: &str, b: &Val) -> Option<bool> {
    match (a, b) {
        (Val::Int(x), Val::Int(y)) => Some(match op { "=="=>x==y, "!="=>x!=y, "<"=>x<y, ">"=>x>y, "<="=>x<=y, ">="=>x>=y, _=>return None }),
        (Val::Str(x), Val::Str(y)) => Some(match op { "=="=>x==y, "!="=>x!=y, "<"=>x<y, ">"=>x>y, "<="=>x<=y, ">="=>x>=y, _=>return None }),
        (Val::Bool(x), Val::Bool(y)) => Some(match op { "=="=>x==y, "!="=>x!=y, _=>return None }),
        _ => None,
    }
}

fn display_root_file(root_path: &Path) -> String {
    if root_path.is_file() { root_path.to_string_lossy().to_string() } else { root_path.to_string_lossy().to_string() }
}
