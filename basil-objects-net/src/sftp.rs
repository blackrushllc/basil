...
pub fn register<F: FnMut(&str, crate::TypeInfo)>(reg: &mut F) {
    ...
    let factory = |args: &[Value]| -> Result<Rc<RefCell<dyn BasicObject>>> {
        ...
        if args.len() < 2 {
            return Err(BasilError("NET_SFTP requires at least host$, user$".into()));
        }
        ...
        if args.len() >= 5 {
            port = match &args[4] {
                Value::Int(i) => (*i as i64).max(0) as u16,
                Value::Num(n) => n.trunc().max(0.0) as u16,
                _ => 22,
            };
        }
        Ok(Rc::new(RefCell::new(NetSftp {
            host,
            user,
            pass,
            keyfile,
...
impl NetSftp {
    fn connect(&self) -> Result<(ssh2::Session, ssh2::Sftp)> {
        ...
        if !self.keyfile.is_empty() {
            ...
            let passphrase = if self.pass.is_empty() {
                None
            } else {
                Some(self.pass.as_str())
            };
            sess.userauth_pubkey_file(&self.user, None, key, passphrase)
                .map_err(|e| err("Connect(auth pubkey)", e))?;
        } else if !self.pass.is_empty() {
            sess.userauth_password(&self.user, &self.pass)
                .map_err(|e| err("Connect(auth password)", e))?;
        } else {
            return Err(BasilError(
                "SFTP.Connect: missing credentials — provide pass$ or keyfile$".into(),
            ));
        }
        if !sess.authenticated() {
            return Err(BasilError("SFTP.Connect: authentication failed".into()));
        }
...
impl BasicObject for NetSftp {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("NET_SFTP has no readable properties".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("NET_SFTP has no settable properties".into()))
    }
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            "PUT$" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "NET_SFTP.Put$ expects local_file$, remote_path$".into(),
                    ));
                }
                ...
                let remote = norm_path(&match &args[1] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                });
                ...
                let mut rf = sftp
                    ...
                    .map_err(|e| err("Put(create remote)", e))?;
                ...
                loop {
                    ...
                    rf.write_all(&buf[..n])
                        .map_err(|e| err("Put(write remote)", e))?;
                }
                Ok(Value::Int(1))
            }
            "GETTOFILE" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "NET_SFTP.GetToFile expects remote_path$, local_file$".into(),
                    ));
                }
                let remote = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                });
                ...
                let mut rf = sftp
                    ...
                    .map_err(|e| err("GetToFile(open remote)", e))?;
                let mut lf = File::create(&local).map_err(|e| err("GetToFile(create local)", e))?;
                let mut buf = [0u8; 64 * 1024];
                loop {
                    let n = rf
                        .read(&mut buf)
                        .map_err(|e| err("GetToFile(read remote)", e))?;
                    if n == 0 {
                        break;
                    }
                    lf.write_all(&buf[..n])
                        .map_err(|e| err("GetToFile(write local)", e))?;
                }
                Ok(Value::Int(1))
            }
            "LIST$" => {
                if args.len() != 1 {
                    return Err(BasilError("NET_SFTP.List$ expects remote_path$".into()));
                }
                let remote = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "MKDIR" => {
                if args.len() != 1 {
                    return Err(BasilError("NET_SFTP.Mkdir expects remote_path$".into()));
                }
                let remote = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                ...
                sftp.mkdir(Path::new(&remote), 0o755)
                    .map_err(|e| err("Mkdir", e))?;
                Ok(Value::Int(1))
            }
            "RMDIR" => {
                if args.len() != 1 {
                    return Err(BasilError("NET_SFTP.Rmdir expects remote_path$".into()));
                }
                let remote = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                ...
                sftp.rmdir(Path::new(&remote))
                    .map_err(|e| err("Rmdir", e))?;
                Ok(Value::Int(1))
            }
            "DELETE" => {
                if args.len() != 1 {
                    return Err(BasilError("NET_SFTP.Delete expects remote_path$".into()));
                }
                let remote = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "RENAME" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "NET_SFTP.Rename expects old_path$, new_path$".into(),
                    ));
                }
                let oldp = norm_path(&match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                });
                let newp = norm_path(&match &args[1] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                });
                let (_sess, sftp) = self.connect()?;
                sftp.rename(Path::new(&oldp), Path::new(&newp), None)
                    .map_err(|e| err("Rename", e))?;
                Ok(Value::Int(1))
            }
            other => Err(BasilError(format!(
                "Unknown method '{}' on NET_SFTP",
                other
            ))),
        }
    }
    ...
    ObjectDescriptor {
        ...
        methods: vec![
            ...
            MethodDesc {
                name: "Put$".into(),
                arity: 2,
                arg_names: vec!["local_file$".into(), "remote_path$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "GetToFile".into(),
                arity: 2,
                arg_names: vec!["remote_path$".into(), "local_file$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "List$".into(),
                arity: 1,
                arg_names: vec!["remote_path$".into()],
            ...
            MethodDesc {
                name: "Rename".into(),
                arity: 2,
                arg_names: vec!["old_path$".into(), "new_path$".into()],
                return_type: "Int (ok%)".into(),
            },
        ],
        examples: vec!["DIM sftp@ AS NET_SFTP(\"host\", \"user\", \"pass\", \"\", 22)".into()],
    }
}
