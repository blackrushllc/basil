...
pub fn register<F: FnMut(&str, crate::TypeInfo)>(reg: &mut F) {
    ...
    let factory = |args: &[Value]| -> Result<Rc<RefCell<dyn BasicObject>>> {
        ...
        if args.len() < 1 {
            return Err(BasilError("MAIL_SMTP requires at least host$".into()));
        }
...
impl SmtpObj {
    fn ensure_transport(&mut self) -> Result<()> {
        ...
        let mut builder = match self.tls_mode.as_str() {
            "tls" => lettre::AsyncSmtpTransport::<Tokio1Executor>::relay(&self.host)
                .map_err(|e| err("Builder(tls)", e))?,
            "plain" => lettre::AsyncSmtpTransport::<Tokio1Executor>::builder_dangerous(&self.host),
            _ => lettre::AsyncSmtpTransport::<Tokio1Executor>::starttls_relay(&self.host)
                .map_err(|e| err("Builder(starttls)", e))?,
        };
...
impl BasicObject for SmtpObj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("MAIL_SMTP has no readable properties".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("MAIL_SMTP has no settable properties".into()))
    }
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "SENDEMAIL" => {
                if !(args.len() == 3 || args.len() == 5) {
                    return Err(BasilError(
                        "MAIL_SMTP.SendEmail expects to$, subject$, body$, from$?, is_html%?"
                            .into(),
                    ));
                }
                let to = match &args[0] {
                ...
                if from_addr.is_empty() {
                    return Err(BasilError(
                        "SMTP.SendEmail: missing from$ and no user$ provided".into(),
                    ));
                }
                let message = {
                    let mb = Message::builder()
                        .from(from_addr.parse::<Mailbox>().map_err(|e| err("From", e))?)
            ...
            "SENDRAW$" => {
                if args.len() != 1 {
                    return Err(BasilError("MAIL_SMTP.SendRaw$ expects mime$".into()));
                }
                ...
                for line in raw.lines() {
                    ...
                    if lcase.starts_with("to:") {
                        let rest = l[3..].trim();
                        for p in rest.split(',') {
                            let a = p.trim().trim_matches('<').trim_matches('>').to_string();
                            if !a.is_empty() {
                                to_addrs.push(a);
                            }
                        }
                ...
                if from_addr.is_empty() || to_addrs.is_empty() {
                    return Err(BasilError(
                        "SMTP.SendRaw$: MIME must include From: and To: headers".into(),
                    ));
                }
                ...
                let resp = TOKIO_RT
                    .block_on(async move { tx.send_raw(&env, &bytes).await })
                    .map_err(|e| err("SendRaw$", e))?;
                let msg = resp.message().collect::<Vec<_>>().join(" ");
                if msg.to_ascii_lowercase().contains("id=") {
                    Ok(Value::Str(msg))
                } else {
            ...
            "MAKEMIME$" => {
                ...
                if args.len() < 3 || args.len() > 6 {
                    return Err(BasilError("MAIL_SMTP.MakeMime$ expects from$, to$, subject$, text_body$?, html_body$?, attach_path$?".into()));
                }
