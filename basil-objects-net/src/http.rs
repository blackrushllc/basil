    ...
    fn rebuild_client(&mut self) -> Result<()> {
        ...
        if !hdrs.contains_key(reqwest::header::AUTHORIZATION) {
            if let Some(tok) = &self.auth_bearer {
                if !tok.is_empty() {
                    let v = reqwest::header::HeaderValue::from_str(&format!("Bearer {}", tok))
                        .map_err(|e| err("Auth(Bearer)", e))?;
                ...
                let v = reqwest::header::HeaderValue::from_str(&format!("Basic {}", cred))
                    .map_err(|e| err("Auth(Basic)", e))?;
                hdrs.insert(reqwest::header::AUTHORIZATION, v);
            }
        }
        if !hdrs.is_empty() {
            ...
            Ok(resp) => {
                ...
                match bytes_res {
                    Ok(bytes) => {
                        ...
                        let mut f =
                            fs::File::create(&out_path).map_err(|e| err("CreateFile", e))?;
...
impl BasicObject for HttpObj {
    ...
    fn set_prop(&mut self, name: &str, v: Value) -> Result<()> {
        match name.to_ascii_uppercase().as_str() {
            ...
            other => Err(BasilError(format!(
                "Unknown or read-only property '{}' on HTTP",
                other
            ))),
        }
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            "SETHEADER" => {
                if args.len() != 2 {
                    return Err(BasilError("HTTP.SetHeader expects name$, value$".into()));
                }
                ...
                let hname = reqwest::header::HeaderName::from_bytes(name.as_bytes())
                    .map_err(|e| err("HeaderName", e))?;
                let hval = reqwest::header::HeaderValue::from_str(&value)
                    .map_err(|e| err("HeaderValue", e))?;
            ...
            "SETBEARER$" => {
                if args.len() != 1 {
                    return Err(BasilError("HTTP.SetBearer$ expects token$".into()));
                }
            ...
            "SETBASICAUTH$" => {
                if args.len() != 2 {
                    return Err(BasilError("HTTP.SetBasicAuth$ expects user$, pass$".into()));
                }
            ...
            "SETQUERYPARAM" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "HTTP.SetQueryParam expects name$, value$".into(),
                    ));
                }
                let name = match &args[0] {
                    Value::Str(s) => s.clone(),
            ...
            "GET$" => {
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(BasilError("HTTP.Get$ expects url$, timeout_ms%?".into()));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                };
            ...
            "DELETE$" => {
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(BasilError("HTTP.Delete$ expects url$, timeout_ms%?".into()));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "HEAD$" | "HEAD" => {
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(BasilError("HTTP.Head$ expects url$, timeout_ms%?".into()));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "POST$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.Post$ expects url$, body$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let body =
                    self.do_request_body(reqwest::Method::POST, url, Some(body), false, to)?;
                Ok(Value::Str(body))
            }
            "PUT$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.Put$ expects url$, body$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let to = Self::parse_timeout_override(args, 2);
                let body =
                    self.do_request_body(reqwest::Method::PUT, url, Some(body), false, to)?;
                Ok(Value::Str(body))
            }
            "PATCH$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.Patch$ expects url$, body$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let to = Self::parse_timeout_override(args, 2);
                let body =
                    self.do_request_body(reqwest::Method::PATCH, url, Some(body), false, to)?;
                Ok(Value::Str(body))
            }
            // JSON convenience
            ,
            "POSTJSON$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.PostJson$ expects url$, json$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let to = Self::parse_timeout_override(args, 2);
                let body =
                    self.do_request_body(reqwest::Method::POST, url, Some(body), true, to)?;
                Ok(Value::Str(body))
            }
            "PUTJSON$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.PutJson$ expects url$, json$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
            ...
            "PATCHJSON$" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.PatchJson$ expects url$, json$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let to = Self::parse_timeout_override(args, 2);
                let body =
                    self.do_request_body(reqwest::Method::PATCH, url, Some(body), true, to)?;
                Ok(Value::Str(body))
            }
            ...
            "DOWNLOADTOFILE" => {
                if !(args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "HTTP.DownloadToFile expects url$, out_path$, timeout_ms%?".into(),
                    ));
                }
                let input_url = match &args[0] {
                    Value::Str(s) => s.clone(),
            ...
            "UPLOADFILE$" => {
                if !(args.len() == 2 || args.len() == 3 || args.len() == 4) {
                    return Err(BasilError(
                        "HTTP.UploadFile$ expects url$, file_path$, field_name$?, content_type$?"
                            .into(),
                    ));
                }
                let input_url = match &args[0] {
    ...
    ObjectDescriptor {
        ...
        methods: vec![
            MethodDesc {
                name: "SetHeader".into(),
                arity: 2,
                arg_names: vec!["name$".into(), "value$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "ClearHeaders".into(),
                arity: 0,
                arg_names: vec![],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "SetBearer$".into(),
                arity: 1,
                arg_names: vec!["token$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "SetBasicAuth$".into(),
                arity: 2,
                arg_names: vec!["user$".into(), "pass$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "SetQueryParam".into(),
                arity: 2,
                arg_names: vec!["name$".into(), "value$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "ClearQueryParams".into(),
                arity: 0,
                arg_names: vec![],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "Get$".into(),
                arity: 2,
                arg_names: vec!["url$".into(), "timeout_ms%?".into()],
                return_type: "String (body$)".into(),
            },
            MethodDesc {
                name: "Delete$".into(),
                arity: 2,
            ...
            MethodDesc {
                name: "Post$".into(),
                arity: 3,
                arg_names: vec!["url$".into(), "body$".into(), "timeout_ms%?".into()],
                return_type: "String (body$)".into(),
            },
            MethodDesc {
                name: "Put$".into(),
            ...
            MethodDesc {
                name: "PostJson$".into(),
                arity: 3,
                arg_names: vec!["url$".into(), "json$".into(), "timeout_ms%?".into()],
                return_type: "String (body$)".into(),
            },
            MethodDesc {
                name: "PutJson$".into(),
            ...
            MethodDesc {
                name: "DownloadToFile".into(),
                arity: 3,
                arg_names: vec!["url$".into(), "out_path$".into(), "timeout_ms%?".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "UploadFile$".into(),
                arity: 4,
                arg_names: vec![
                    "url$".into(),
                    "file_path$".into(),
        ...
        examples: vec!["DIM http@ AS HTTP()".into()],
    }
}
