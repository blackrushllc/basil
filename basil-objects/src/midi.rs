...
fn in_tab() -> &'static Mutex<Vec<Option<InPort>>> {
    IN_TABLE.get_or_init(|| Mutex::new(Vec::new()))
}
...
pub fn midi_open_in(substr: &str) -> Result<i64> {
    let mut midi_in = midir::MidiInput::new("basil-midi-in")
        .map_err(|e| BasilError(format!("MIDI_OPEN_IN@: init failed: {e}")))?;
    midi_in.ignore(midir::Ignore::None);
    let ports = midi_in.ports();
    if ports.is_empty() {
        return Err(BasilError(
            "MIDI_OPEN_IN@: no MIDI input ports available".into(),
        ));
    }
...
pub fn midi_poll(h: i64) -> Result<i64> {
    ...
    let port = tab
        .get(h as usize)
        .and_then(|o| o.as_ref())
        .ok_or_else(|| BasilError("Invalid MIDI handle".into()))?;
    let q = port
        .q
        .lock()
        .map_err(|_| BasilError("MIDI queue lock poisoned".into()))?;
    Ok(q.len() as i64)
}
...
pub fn midi_get_event(h: i64) -> Result<(u8, u8, u8)> {
    ...
    if let Some(ev) = q.pop_front() {
        Ok((ev.status, ev.d1, ev.d2))
    } else {
        Err(BasilError("No MIDI events".into()))
    }
}
