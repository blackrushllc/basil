...
pub fn register(reg: &mut crate::Registry) {
    ...
    reg.register(
        "BASE64",
        crate::TypeInfo {
            factory: |_args| Ok(std::rc::Rc::new(std::cell::RefCell::new(Base64Obj {}))),
...
impl BasicObject for Base64Obj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("BASE64 has no properties".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "ENCODE$" => {
                if args.len() != 1 {
                    return Err(BasilError("Encode$ expects 1 argument".into()));
                }
                let s = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "DECODE$" => {
                if args.len() != 1 {
                    return Err(BasilError("Decode$ expects 1 argument".into()));
                }
                let s = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                ...
                #[cfg(feature = "base64")]
                {
                    match general_purpose::STANDARD.decode(s) {
                        Ok(bytes) => match String::from_utf8(bytes) {
                            ...
                            Err(_) => Err(BasilError(
                                "BASE64.Decode$: invalid UTF-8 in decoded data".into(),
                            )),
                        },
