...
pub fn stop() {
    crate::audio::daw_stop();
}
pub fn stop_clear() {
    crate::audio::daw_stop_clear();
}
pub fn should_stop() -> bool {
    crate::audio::daw_should_stop()
}
...
pub fn audio_record(input_sub: &str, out_path: &str, seconds: i64) -> i64 {
    stop_clear();
    if seconds <= 0 {
        set_err("AUDIO_RECORD%: seconds must be > 0");
        return 1;
    }
    // Open input
    ...
    let (rate, chans) = match crate::audio::audio_in_params(in_h) {
        Ok(rc) => rc,
        Err(_) => (
            crate::audio::audio_default_rate() as u32,
            crate::audio::audio_default_chans() as u16,
        ),
    };
    // Ring to bridge RT -> writer
    let cap_samples = (rate as usize)
        .saturating_mul(chans as usize)
        .saturating_mul(seconds as usize)
        .saturating_mul(2);
    let ring_h = match crate::audio::ring_create(cap_samples as i64) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("AUDIO_RECORD%: {}", e));
    ...
    if let Err(e) = crate::audio::audio_connect_in_to_ring(in_h, ring_h) {
        set_err(format!("AUDIO_RECORD%: {}", e));
        let _ = crate::audio::audio_close(in_h);
        return 1;
    }
    if let Err(e) = crate::audio::audio_start(in_h) {
        set_err(format!("AUDIO_RECORD%: {}", e));
        let _ = crate::audio::audio_close(in_h);
        return 1;
    }
    // WAV writer
    let wr_h = match crate::audio::wav_writer_open(out_path, rate as i64, chans as i64) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("AUDIO_RECORD%: {}", e));
    ...
    while written < target {
        ...
        let n = match crate::audio::ring_pop(ring_h, &mut buf) {
            Ok(n) => n as usize,
            Err(_) => 0,
        };
        if n > 0 {
            if let Err(e) = crate::audio::wav_writer_write(wr_h, &buf[..n]) {
                set_err(format!("AUDIO_RECORD%: {}", e));
...
pub fn audio_play(output_sub: &str, file_path: &str) -> i64 {
    ...
    let mut reader = match hound::WavReader::open(file_path) {
        Ok(r) => r,
        Err(e) => {
            set_err(format!("AUDIO_PLAY%: failed to open WAV: {e}"));
            return 1;
        }
    };
        ...
        hound::SampleFormat::Int => {
            for s in reader.samples::<i16>() {
                match s {
                    ...
                    Err(e) => {
                        set_err(format!("AUDIO_PLAY%: WAV read failed: {e}"));
                        return 1;
                    }
                }
            }
    ...
    let device = if output_sub.trim().is_empty() {
        match host.default_output_device() {
            Some(d) => d,
            None => {
                set_err("AUDIO_PLAY%: no default output device");
                return 1;
            }
        }
    } else {
        ...
        match found.or_else(|| host.default_output_device()) {
            ...
            None => {
                set_err(format!(
                    "AUDIO_PLAY%: no output device matches '{output_sub}'"
                ));
                return 1;
            }
        }
    ...
    let supported = match device.default_output_config() {
        Ok(c) => c,
        Err(e) => {
            set_err(format!("AUDIO_PLAY%: failed to get device config: {e}"));
            return 1;
        }
    };
    ...
    let in_frames = if in_ch == 0 {
        0
    } else {
        in_samples.len() / in_ch
    };
    if in_frames == 0 {
        set_err("AUDIO_PLAY%: WAV has no samples");
        return 1;
    }
    ...
    let stream_res = match sample_format {
        ...
        cpal::SampleFormat::I16 => {
            ...
            device.build_output_stream(
                ...
                move |buf: &mut [i16], _| {
                    ...
                    for s in buf.iter_mut() {
                        if i < data2.len() {
                            *s = (data2[i].max(-1.0).min(1.0) * i16::MAX as f32) as i16;
                            i += 1;
                        } else {
                            *s = 0;
                        }
        ...
        _ => {
            set_err("AUDIO_PLAY%: unsupported device sample format");
            return 1;
        }
    };
    ...
    let stream = match stream_res {
        Ok(s) => s,
        Err(e) => {
            set_err(format!("AUDIO_PLAY%: failed to build stream: {e}"));
            return 1;
        }
    };

    if let Err(e) = stream.play() {
        set_err(format!("AUDIO_PLAY%: failed to start stream: {e}"));
        return 1;
    }
...
pub fn audio_monitor(input_sub: &str, output_sub: &str) -> i64 {
    ...
    let in_h = match crate::audio::audio_open_in(input_sub) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("AUDIO_MONITOR%: {}", e));
            return 1;
        }
    };
    let out_h = match crate::audio::audio_open_out(output_sub) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("AUDIO_MONITOR%: {}", e));
            let _ = crate::audio::audio_close(in_h);
    ...
    let (rate, chans) = match crate::audio::audio_out_params(out_h) {
        Ok(rc) => rc,
        Err(_) => (
            crate::audio::audio_default_rate() as u32,
            crate::audio::audio_default_chans() as u16,
    ...
    if cap_samples < 128 * (chans as usize).max(1) {
        cap_samples = 128 * (chans as usize).max(1);
    }
    let ring_h = match crate::audio::ring_create(cap_samples as i64) {
        Ok(h) => h,
        Err(e) => {
    ...
    if let Err(e) = crate::audio::audio_connect_ring_to_out(ring_h, out_h) {
        set_err(format!("AUDIO_MONITOR%: {}", e));
        let _ = crate::audio::audio_close(in_h);
        let _ = crate::audio::audio_close(out_h);
        return 1;
    ...
    if let Err(e) = crate::audio::audio_start(out_h) {
        set_err(format!("AUDIO_MONITOR%: {}", e));
        let _ = crate::audio::audio_stop(in_h);
        let _ = crate::audio::audio_close(in_h);
        let _ = crate::audio::audio_close(out_h);
...
pub fn midi_capture(port_sub: &str, out_jsonl: &str) -> i64 {
    stop_clear();
    // Open MIDI input
    let in_h = match crate::midi::midi_open_in(port_sub) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("MIDI_CAPTURE%: {}", e));
            return 1;
        }
    };
    // Open log file
        ...
        .open(out_jsonl)
    {
        Ok(f) => f,
        Err(e) => {
            set_err(format!("MIDI_CAPTURE%: cannot open file: {e}"));
    ...
    loop {
        ...
        let n = match crate::midi::midi_poll(in_h) {
            Ok(n) => n,
            Err(e) => {
                set_err(format!("MIDI_CAPTURE%: {}", e));
                let _ = crate::midi::midi_close(in_h);
            ...
            for _ in 0..n {
                match crate::midi::midi_get_event(in_h) {
                    Ok((s, d1, d2)) => {
                        ...
                        let line = format!(
                            "{{\"ts\":{ms},\"status\":{s},\"data1\":{d1},\"data2\":{d2}}}\n"
                        );
                        if let Err(e) = std::io::Write::write_all(&mut file, line.as_bytes()) {
                            set_err(format!("MIDI_CAPTURE%: write failed: {e}"));
                            let _ = crate::midi::midi_close(in_h);
                            return 1;
                        }
                    }
...
pub fn synth_live(midi_port_sub: &str, output_sub: &str, poly: i64) -> i64 {
    ...
    // Open MIDI in
    let in_h = match crate::midi::midi_open_in(midi_port_sub) {
        Ok(h) => h,
        Err(e) => {
            set_err(format!("SYNTH_LIVE%: {}", e));
            let _ = crate::audio::audio_stop(out_h);
    ...
    let worker = std::thread::spawn(move || {
        ...
        loop {
            if crate::daw::should_stop() {
                break;
            }
            // Drain MIDI events
            match crate::midi::midi_poll(in_h) {
                Ok(n) => {
                    for _ in 0..n {
