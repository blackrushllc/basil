...
fn conns() -> &'static Mutex<ConnTable> {
    CONNS.get_or_init(|| Mutex::new(ConnTable::default()))
}
...
impl ConnTable {
    ...
    fn get_mut(&mut self, id: i32) -> Option<&mut Connection> {
        self.map.get_mut(&id)
    }
...
pub fn sqlite_exec(handle: i64, sql: &str) -> i64 {
    let mut tbl = conns().lock().unwrap();
    let conn = match tbl.get_mut(handle as i32) {
        Some(c) => c,
        None => return -1,
    };
...
pub fn sqlite_query2d(handle: i64, sql: &str) -> Result<Value> {
    ...
    let conn = tbl
        ...
        .ok_or_else(|| BasilError("SQLITE_QUERY2D$: invalid handle".into()))?;
    ...
    for r in rows_iter {
        ...
        if row.len() != col_cnt {
            return Err(BasilError(
                "SQLITE_QUERY2D$: inconsistent column count".into(),
            ));
        }
        data.extend(row);
        row_cnt += 1;
