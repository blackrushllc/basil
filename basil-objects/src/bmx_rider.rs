    ...
    reg.register(
        ...
        crate::TypeInfo {
            factory: |args| {
                ...
                let skill = match &args[2] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                };
                let wins = match &args[3] {
                    Value::Int(i) => *i,
                    Value::Num(n) => n.trunc() as i64,
                    _ => 0,
                };
                let losses = match &args[4] {
                    Value::Int(i) => *i,
                    Value::Num(n) => n.trunc() as i64,
                    _ => 0,
                };
                Ok(Rc::new(RefCell::new(BmxRider {
                    name,
                    age,
                    skill_level: skill,
                    wins,
...
fn descriptor_static() -> ObjectDescriptor {
    ObjectDescriptor {
        ...
        methods: vec![
            ...
            MethodDesc {
                name: "TotalRaces%".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "Integer".to_string(),
            },
            MethodDesc {
                name: "WinRate".to_string(),
                arity: 0,
                arg_names: vec![],
...
impl BasicObject for BmxRider {
    ...
    fn call(&mut self, method: &str, _args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            ...
            "TAGS$" => Ok(make_str_array(vec![
                self.name.clone(),
                self.skill_level.clone(),
            ])),
            "INFO$" => Ok(Value::Str(format!(
                "Rider {} ({} {})",
                self.name, self.age, self.skill_level
            ))),
            other => Err(BasilError(format!(
                "Unknown method '{}' on BMX_RIDER",
                other
            ))),
        }
    }
