...
use basil_bytecode::{ArrayObj, BasicObject, ElemType, ObjectDescriptor, Value};
...
impl BasicObject for ConvManager {
    ...
    fn get_prop(&self, name: &str) -> Result<Value> {
        Err(BasilError(format!(
            "AI.CONVERSATION has no property '{}'",
            name
        )))
    }
    ...
    fn set_prop(&mut self, name: &str, _v: Value) -> Result<()> {
        Err(BasilError(format!(
            "AI.CONVERSATION has no settable property '{}'",
            name
        )))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        let ai_rc = self
            .ai
            .upgrade()
            .ok_or_else(|| BasilError("AI object no longer exists".into()))?;
        ...
        match method.to_ascii_uppercase().as_str() {
            "CREATE$" => {
                ...
                if args.len() >= 1 {
                    let opts_str = match &args[0] {
                        Value::Str(s) => s.clone(),
                        other => format!("{}", other),
                    };
                    if let Ok(opts) = serde_json::from_str::<serde_json::Value>(&opts_str) {
                        if let Some(b) = opts.get("backend").and_then(|v| v.as_str()) {
                            backend = b.to_string();
                        }
                    }
                }
                if backend == "openai" && resolve_api_key().is_none() {
                        ...
                        Some(api_key) => {
                            match openai_request(
                                ...
                                Ok(v) => {
                                    ...
                                    if id.is_empty() {
                                        return Err(
                                            ai.set_err("OpenAI did not return a conversation ID")
                                        );
                                    }
            ...
            "DELETE" => {
                ...
                if args.len() < 1 {
                    return Err(ai.set_err("CONVERSATION.DELETE expects 1 argument"));
                }
                let id = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                ...
                if let Some(entry) = entry_to_delete {
                    if entry.backend == "openai" {
                        if let Some(api_key) = resolve_api_key() {
                            ...
                            loop {
                                let path = format!(
                                    "/v1/conversations/{}/items?limit=100&order=desc{}",
                                    id,
                                    after
                                        .as_ref()
                                ...
                                match openai_request("GET", &path, None, &api_key, 30000) {
                                    Ok(v) => {
                                        if let Some(items) =
                                            v.get("data").and_then(|a| a.as_array())
                                        {
                                            for item in items {
                                                if let Some(iid) =
                                                    item.get("id").and_then(|s| s.as_str())
                                                {
                                                    let _ = openai_request(
                                                        "DELETE",
                                                        &format!(
                                        ...
                                        if let Some(has_more) =
                                            v.get("has_more").and_then(|b| b.as_bool())
                                        {
                                            if has_more {
                                                after = v
                                                    .get("last_id")
            ...
            other => Err(BasilError(format!(
                "Unknown method '{}' on AI.CONVERSATION",
                other
            ))),
        }
    }
    ...
    fn descriptor(&self) -> ObjectDescriptor {
        basil_bytecode::ObjectDescriptor {
            ...
            properties: vec![],
            methods: vec![
                basil_bytecode::MethodDesc {
                    name: "CREATE$".into(),
                    arity: 1,
                    arg_names: vec!["opts$".into()],
...
where
    F: FnOnce(&mut ConversationRegistry) -> std::result::Result<R, String>,
{
    ...
    let _lock_file = loop {
            ...
            .open(&lock_path)
        {
            ...
            Err(_) => {
                if retry > 20 {
                    return Err("could not acquire registry lock".into());
                }
        ...
        match method.to_ascii_uppercase().as_str() {
            "CHAT$" => {
                ...
                if !(args.len() >= 1 && args.len() <= 3) {
                    return Err(self.set_err("AI.CHAT$ expects 1 to 3 arguments"));
                }
                ...
                if args.len() == 2 {
                    ...
                    if !arg1.is_empty() {
                        if arg1.starts_with("conv_") {
                            conversation_id = Some(arg1);
                        } else {
                            options_json = Some(arg1);
                        }
                    }
                } else if args.len() == 3 {
                    let arg1 = match &args[1] {
                    ...
                    options_json = Some(match &args[2] {
                        Value::Str(s) => s.clone(),
                        other => format!("{}", other),
                    });
                }
                        ...
                        if let Some(cid) = opts.get("conversation_id").and_then(|v| v.as_str()) {
                            if conversation_id.is_none() {
                                ...
                                return Err(self.set_err(
                                    "Mismatched conversation_id in arguments and options",
                                ));
                            }
                        }
                ...
                if let Some(opts_str) = &options_json {
                    if let Ok(opts) = serde_json::from_str::<serde_json::Value>(opts_str) {
                        if let Some(prid) =
                            opts.get("previous_response_id").and_then(|v| v.as_str())
                        {
                ...
                if backend == "local" {
                    ...
                    let mut local_conv = if path.exists() {
                        ...
                        serde_json::from_str::<LocalConversation>(&s).unwrap_or(LocalConversation {
                            messages: Vec::new(),
                        })
                    } else {
                        LocalConversation {
                    ...
                    local_conv.messages.push(ChatMessage {
                        role: "user".into(),
                        ...
                        Some(api_key) => {
                            ...
                            if let Some(opts_str) = &options_json {
                                    ...
                                    serde_json::from_str::<serde_json::Value>(opts_str)
                                {
                                    ...
                                    if let Some(i) = opts.get("instructions") {
                                        body["instructions"] = i.clone();
                                    }
                                }
                            }
                            match openai_request(
                                ...
                                Ok(v) => {
                                    ...
                                    let _ = fs::write(
                                        &path,
                                        serde_json::to_string_pretty(&local_conv)
                                            .unwrap_or_default(),
                                    );
                                    self.last_response_id = v
                                        .get("id")
                                        .and_then(|s| s.as_str())
                                        .unwrap_or("")
                                        .to_string();
                                    ...
                                    let _ = with_registry_lock(|reg| {
                                        if let Some(entry) = reg
                                            .conversations
                                            .iter_mut()
                                            .find(|c| c.id == self.last_conversation_id)
                                        {
            ...
            "STREAM" => {
                ...
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(self.set_err("AI.STREAM expects 1 or 2 arguments"));
                }
                let prompt = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "EMBED" => {
                self.clear_err();
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(self.set_err("AI.EMBED expects 1 or 2 arguments"));
                }
                ...
                match resolve_api_key() {
                    Some(api_key) => match embed_request(&text, &api_key) {
                        Ok(vec) => {
                            let dims = vec![vec.len()];
                            let data: Vec<Value> = vec.iter().copied().map(Value::Num).collect();
                            let arr = ArrayObj {
            ...
            "MODERATE%" => {
                self.clear_err();
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(self.set_err("AI.MODERATE% expects 1 or 2 arguments"));
                }
                let text = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "KNOWLEDGE$" => {
                self.clear_err();
                if !(args.len() == 1 || args.len() == 2) {
                    return Err(self.set_err("AI.KNOWLEDGE$ expects 1 or 2 arguments"));
                }
                ...
                match fs::read_to_string(&path) {
                    Ok(s) => Ok(Value::Str(s)),
                    Err(e) => {
                        self.set_err(format!("read failed: {}", e));
                        Ok(Value::Str(String::new()))
                    }
                }
            }
...
fn descriptor_static() -> ObjectDescriptor {
    basil_bytecode::ObjectDescriptor {
        ...
        properties: vec![basil_bytecode::PropDesc {
        ...
        methods: vec![
            ...
            basil_bytecode::MethodDesc {
                ...
                arity: 2,
                arg_names: vec!["prompt$".to_string(), "opts$".to_string()],
                return_type: "String".to_string(),
            },
            basil_bytecode::MethodDesc {
                name: "EMBED".to_string(),
                arity: 2,
                arg_names: vec!["text$".to_string(), "opts$".to_string()],
            ...
            basil_bytecode::MethodDesc {
                ...
                arity: 2,
                arg_names: vec!["path$".to_string(), "opts$".to_string()],
                return_type: "String".to_string(),
            },
            basil_bytecode::MethodDesc {
                name: "LAST_RESPONSE_ID$".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "String".to_string(),
            },
            basil_bytecode::MethodDesc {
                name: "LAST_CONVERSATION_ID$".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "String".to_string(),
            },
            basil_bytecode::MethodDesc {
                name: "LAST_META$".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "String".to_string(),
            },
...
fn test_embed16(text: &str) -> Vec<f64> {
    ...
    #[cfg(feature = "sha1")]
    {
        ...
        for i in 0..16 {
            let b = d[i % d.len()] as i32;
            let v = (b - 128) as f64 / 128.0;
            out.push(v);
        }
        return out;
    }
    ...
    {
        for i in 0..16 {
            out.push(((i as i32 - 8) as f64) / 8.0);
        }
        out
    }
}
...
fn responses_request(
    ...
    if let Some(opts_str) = opts_json {
        if let Ok(opts) = serde_json::from_str::<serde_json::Value>(opts_str) {
            ...
            if let Some(m) = opts.get("max_output_tokens") {
                body["max_output_tokens"] = m.clone();
            }
            if let Some(s) = opts.get("store") {
                body["store"] = s.clone();
            }
            if let Some(meta) = opts.get("metadata") {
