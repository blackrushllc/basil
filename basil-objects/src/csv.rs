...
pub fn write_from_rows_json(rows_json: &str) -> Result<String> {
    ...
    let arr = rows
        .as_array()
        .ok_or_else(|| BasilError("CSV_WRITE$: expected JSON array of objects".into()))?;
    ...
    for v in arr.iter() {
        let obj = v
            .as_object()
            .ok_or_else(|| BasilError("CSV_WRITE$: array items must be objects".into()))?;
    ...
    let bytes = wtr
        .into_inner()
        .map_err(|e| BasilError(format!("CSV_WRITE$: finalize failed: {}", e)))?;
