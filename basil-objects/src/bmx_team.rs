...
pub fn register(reg: &mut crate::Registry) {
    reg.register(
        ...
        crate::TypeInfo {
            factory: |args| {
                ...
                if args.len() < 2 || args.len() > 3 {
                    return Err(BasilError(format!(
                        "BMX_TEAM expects 2 or 3 args, got {}",
                        args.len()
                    )));
                }
            ...
            constants: || {
                vec![
                    ("PRO".to_string(), Value::Int(1)),
                    ("NOT_PRO".to_string(), Value::Int(0)),
                ]
            },
        },
    );
...
fn descriptor_static() -> ObjectDescriptor {
    ObjectDescriptor {
        ...
        properties: vec![
            ...
            PropDesc {
                name: "EstablishedYear%".to_string(),
                type_name: "Integer".to_string(),
                readable: true,
                writable: true,
            },
            PropDesc {
                name: "TeamWins%".to_string(),
                type_name: "Integer".to_string(),
                readable: true,
                writable: true,
            },
            PropDesc {
                name: "TeamLosses%".to_string(),
                type_name: "Integer".to_string(),
                readable: true,
                writable: true,
            },
        ...
        methods: vec![
            MethodDesc {
                name: "AddRider".to_string(),
                arity: 1,
                arg_names: vec!["rider@".to_string()],
                return_type: "Void".to_string(),
            },
            MethodDesc {
                name: "RemoveRider".to_string(),
                arity: 1,
            ...
            MethodDesc {
                name: "TopRider".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "BMX_RIDER".to_string(),
            },
            MethodDesc {
                name: "BottomRider".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "BMX_RIDER".to_string(),
            },
            MethodDesc {
                name: "RiderNames$".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "String[]".to_string(),
            },
            MethodDesc {
                name: "RiderDescriptions$".to_string(),
                arity: 0,
                arg_names: vec![],
                return_type: "String[]".to_string(),
            },
...
fn make_str_array(items: Vec<String>) -> Value {
    use basil_bytecode::{ArrayObj, ElemType};
...
impl BasicObject for BmxTeam {
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "ADDRIDER" => {
                if args.len() != 1 {
                    return Err(BasilError("AddRider expects 1 arg".into()));
                }
                match &args[0] {
                    Value::Object(r) => {
                        self.roster.push(r.clone());
                        Ok(Value::Null)
                    }
                    _ => Err(BasilError("AddRider expects a BMX_RIDER object".into())),
                }
            }
            "REMOVERIDER" => {
                if args.len() != 1 {
                    return Err(BasilError("RemoveRider expects 1 arg".into()));
                }
                let name = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "WINPCT" => {
                ...
                if total <= 0 {
                    ...
                    Ok(Value::Num(self.team_wins as f64 / total as f64))
                }
            }
            "TOPRIDER" => {
                if self.roster.is_empty() {
                    return Err(BasilError("TopRider: roster empty".into()));
                }
                ...
                for (i, r) in self.roster.iter().enumerate() {
                    let wr = match r.borrow_mut().call("WinRate", &[]) {
                        Ok(Value::Num(n)) => n,
                        _ => 0.0,
                    };
                    let wins = match r.borrow().get_prop("Wins%") {
                        Ok(Value::Int(i)) => i,
                        _ => 0,
                    };
                    let age = match r.borrow().get_prop("Age%") {
                        Ok(Value::Int(i)) => i,
                        _ => 0,
                    };
                    if wr > best_wrp
                        || (wr == best_wrp
                            && (wins > best_wins || (wins == best_wins && age < best_age)))
                    {
                        best_idx = i;
                        best_wrp = wr;
                        best_wins = wins;
            ...
            "BOTTOMRIDER" => {
                if self.roster.is_empty() {
                    return Err(BasilError("BottomRider: roster empty".into()));
                }
                ...
                for (i, r) in self.roster.iter().enumerate() {
                    ...
                    if wr < worst_wrp
                        || (wr == worst_wrp
                            && (wins < worst_wins || (wins == worst_wins && age > worst_age)))
                    {
                        worst_idx = i;
                        worst_wrp = wr;
                        worst_wins = wins;
            ...
            "RIDERNAMES$" => {
                let names = self
                    .roster
                    .iter()
                    .map(|r| match r.borrow().get_prop("Name$") {
                        Ok(Value::Str(s)) => s,
                        _ => String::new(),
                    })
                    .collect::<Vec<_>>();
            ...
            "RIDERDESCRIPTIONS$" => {
                let descs = self
                    .roster
                    .iter()
                    .map(|r| match r.borrow_mut().call("Describe$", &[]) {
                        Ok(Value::Str(s)) => s,
                        _ => String::new(),
                    })
                    .collect::<Vec<_>>();
            ...
            "INFO$" => Ok(Value::Str(format!(
                "Team {} (est. {}, {}), {} riders",
                self.name,
                self.established_year,
                if self.is_pro { "Pro" } else { "Amateur" },
                self.roster.len()
            ))),
            other => Err(BasilError(format!(
                "Unknown method '{}' on BMX_TEAM",
                other
            ))),
        }
    }
