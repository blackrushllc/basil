...
pub fn set_err(msg: impl Into<String>) {
    LAST_ERR.with(|e| *e.borrow_mut() = msg.into());
}
pub fn take_err() -> String {
...
pub mod wavio {
    ...
    pub fn write_interleaved_f32(path: &str, rate: u32, chans: u16, frames: &[f32]) -> Result<()> {
        ...
        for &s in frames.iter() {
            ...
            wr.write_sample(v)
                .map_err(|e| BasilError(format!("WAV write failed: {e}")))?;
        }
        wr.finalize()
            .map_err(|e| BasilError(format!("WAV finalize failed: {e}")))?;
        Ok(())
    }
    pub fn read_all_f32(path: &str) -> Result<Vec<f32>> {
        let mut rdr = hound::WavReader::open(path)
            .map_err(|e| BasilError(format!("WAV open failed: {e}")))?;
        let spec = rdr.spec();
        let mut out = Vec::new();
        match spec.sample_format {
            hound::SampleFormat::Int => {
                for s in rdr.samples::<i16>() {
                    let v = s.map_err(|e| BasilError(format!("WAV read failed: {e}")))?;
...
pub fn audio_default_rate() -> i64 {
    ...
    if let Some(dev) = host.default_output_device() {
        if let Ok(cfg) = dev.default_output_config() {
            return cfg.sample_rate().0 as i64;
        }
    }
    48000
}
...
pub fn audio_inputs() -> Vec<String> {
    ...
    if let Ok(devs) = host.input_devices() {
        for d in devs {
            v.push(d.name().unwrap_or_default());
        }
    }
    v
}
...
mod devio {
    ...
    pub struct InEntry {
        ...
        pub ring_sel: Arc<Mutex<Option<rtrb::Producer<f32>>>>,
        pub chans: u16,
        pub rate: u32,
    }
    pub struct OutEntry {
        pub stream: cpal::Stream,
        pub ring_sel: Arc<Mutex<Option<rtrb::Consumer<f32>>>>,
        pub chans: u16,
        pub rate: u32,
    }
    ...
    fn in_tab_mut<F: FnOnce(&mut Vec<Option<InEntry>>)>(f: F) {
        IN_TABLE.with(|t| f(&mut *t.borrow_mut()))
    }
    fn out_tab_mut<F: FnOnce(&mut Vec<Option<OutEntry>>)>(f: F) {
        OUT_TABLE.with(|t| f(&mut *t.borrow_mut()))
    }
    ...
    fn find_input(substr: &str) -> Option<cpal::Device> {
        ...
        if let Ok(devs) = host.input_devices() {
            for d in devs {
                if d.name().ok()?.to_lowercase().contains(&needle) {
                    return Some(d);
                }
            }
        }
    ...
    pub fn open_in(substr: &str) -> Result<i64> {
        let dev = find_input(substr).ok_or_else(|| {
            BasilError(format!(
                "AUDIO_OPEN_IN@: no input device matches '{}'",
                substr
            ))
        })?;
        let cfg = dev
            .default_input_config()
            .map_err(|e| BasilError(format!("AUDIO_OPEN_IN@: get config failed: {e}")))?;
        ...
        let stream = match sample_format {
                    ...
                    move |buf: &[u16], _| {
                        if let Ok(mut sel) = rs.lock() {
                            if let Some(prod) = sel.as_mut() {
                                ...
                                for (i, &s) in buf.iter().enumerate() {
                                    scratch[i] = (s as f32 / u16::MAX as f32) * 2.0 - 1.0;
                                }
                                for &v in &scratch[..buf.len()] {
                                    if prod.push(v).is_err() {
                                        break;
            ...
            _ => {
                return Err(BasilError(
                    "AUDIO_OPEN_IN@: unsupported sample format".into(),
                ))
            }
        }
    ...
    pub fn open_out(substr: &str) -> Result<i64> {
        let dev = find_output(substr).ok_or_else(|| {
            BasilError(format!(
                "AUDIO_OPEN_OUT@: no output device matches '{}'",
                substr
            ))
        })?;
        let cfg = dev
            .default_output_config()
            .map_err(|e| BasilError(format!("AUDIO_OPEN_OUT@: get config failed: {e}")))?;
        let sample_format = cfg.sample_format();
        let mut config = cfg.config().clone();
        // Best-effort smaller buffer for lower latency
        config.buffer_size = cpal::BufferSize::Fixed(256);
        ...
        let stream = match sample_format {
            ...
            _ => {
                return Err(BasilError(
                    "AUDIO_OPEN_OUT@: unsupported sample format".into(),
                ))
            }
        }
    ...
    pub fn start(h: i64) -> Result<i64> {
        ...
        in_tab_mut(|tab| {
            if let Some(Some(entry)) = tab.get_mut(h as usize) {
                found = true;
                if let Err(e) = entry.stream.play() {
                    err = Some(format!("{}", e));
                }
            }
        });
        if found {
            return if let Some(e) = err {
                Err(BasilError(format!("AUDIO_START%: {}", e)))
            } else {
                Ok(0)
            };
        }
    ...
    pub fn stop(h: i64) -> Result<i64> {
        ...
        in_tab_mut(|tab| {
            if let Some(Some(entry)) = tab.get_mut(h as usize) {
                found = true;
                if let Err(e) = entry.stream.pause() {
                    err = Some(format!("{}", e));
                }
            }
        });
        if found {
            return if let Some(e) = err {
                Err(BasilError(format!("AUDIO_STOP%: {}", e)))
            } else {
                Ok(0)
            };
        }
    ...
    pub fn close(h: i64) -> Result<i64> {
        ...
        if closed {
            ...
            Err(BasilError("AUDIO_CLOSE%: invalid handle".into()))
        }
    }
    ...
    pub fn in_params(h: i64) -> Result<(u32, u16)> {
        ...
        IN_TABLE.with(|t| {
            let tab = t.borrow();
            if let Some(Some(e)) = tab.get(h as usize) {
                res = Some((e.rate, e.chans));
            }
        });
    ...
    pub fn connect_in_to_ring(in_h: i64, ring_h: i64) -> Result<i64> {
        ...
        let mut prod_opt = {
            ...
            let e = tab
                .get_mut(ring_h as usize)
                .and_then(|o| o.as_mut())
                .ok_or_else(|| BasilError("Invalid ring handle".into()))?;
            e.prod.take()
        };
        let mut ok = false;
        in_tab_mut(|tab| {
        ...
        if ok {
            ...
            Err(BasilError(
                "Invalid input handle or ring producer already connected".into(),
            ))
        }
    }
    pub fn connect_ring_to_out(ring_h: i64, out_h: i64) -> Result<i64> {
        ...
        if ok {
            ...
            Err(BasilError(
                "Invalid output handle or ring consumer already connected".into(),
            ))
        }
    }
}
...
#[cfg(not(feature = "obj-audio"))]
// Device stubs
pub fn audio_open_in(_substr: &str) -> Result<i64> {
    set_err("AUDIO_OPEN_IN@: device I/O not compiled in");
    Err(BasilError("AUDIO_OPEN_IN@: not available".into()))
}
#[cfg(not(feature = "obj-audio"))]
pub fn audio_open_out(_substr: &str) -> Result<i64> {
    set_err("AUDIO_OPEN_OUT@: device I/O not compiled in");
    Err(BasilError("AUDIO_OPEN_OUT@: not available".into()))
}
#[cfg(not(feature = "obj-audio"))]
pub fn audio_start(_h: i64) -> Result<i64> {
    set_err("AUDIO_START%: not available");
    Err(BasilError("AUDIO_START%: not available".into()))
}
#[cfg(not(feature = "obj-audio"))]
pub fn audio_stop(_h: i64) -> Result<i64> {
    Ok(0)
}
#[cfg(not(feature = "obj-audio"))]
pub fn audio_close(_h: i64) -> Result<i64> {
    Ok(0)
}

#[cfg(feature = "obj-audio")]
pub fn audio_open_in(substr: &str) -> Result<i64> {
    devio::open_in(substr)
}
#[cfg(feature = "obj-audio")]
pub fn audio_open_out(substr: &str) -> Result<i64> {
    devio::open_out(substr)
}
#[cfg(feature = "obj-audio")]
pub fn audio_start(h: i64) -> Result<i64> {
    devio::start(h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_stop(h: i64) -> Result<i64> {
    devio::stop(h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_close(h: i64) -> Result<i64> {
    devio::close(h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_connect_in_to_ring(in_h: i64, ring_h: i64) -> Result<i64> {
    devio::connect_in_to_ring(in_h, ring_h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_connect_ring_to_out(ring_h: i64, out_h: i64) -> Result<i64> {
    devio::connect_ring_to_out(ring_h, out_h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_in_params(h: i64) -> Result<(u32, u16)> {
    devio::in_params(h)
}
#[cfg(feature = "obj-audio")]
pub fn audio_out_params(h: i64) -> Result<(u32, u16)> {
    devio::out_params(h)
}
...
struct RbEntry {
    ...
    cons: Option<rtrb::Consumer<f32>>,
}
static RB_TABLE: OnceLock<Mutex<Vec<Option<RbEntry>>>> = OnceLock::new();
fn rb_tab() -> &'static Mutex<Vec<Option<RbEntry>>> {
    RB_TABLE.get_or_init(|| Mutex::new(Vec::new()))
}
...
pub fn ring_push(h: i64, data: &[f32]) -> Result<i64> {
    ...
    let entry = tab
        ...
        .and_then(|o| o.as_mut())
        .ok_or_else(|| BasilError("Invalid ring handle".into()))?;
    let prod = entry.prod.as_mut().ok_or_else(|| {
        BasilError("Ring producer not available (already connected to input)".into())
    })?;
    let mut n = 0i64;
    for &x in data.iter() {
...
pub fn ring_pop(h: i64, out: &mut [f32]) -> Result<i64> {
    ...
    let entry = tab
        ...
        .and_then(|o| o.as_mut())
        .ok_or_else(|| BasilError("Invalid ring handle".into()))?;
    let cons = entry.cons.as_mut().ok_or_else(|| {
        BasilError("Ring consumer not available (already connected to output)".into())
    })?;
    let mut n = 0i64;
    for o in out.iter_mut() {
        if let Ok(x) = cons.pop() {
            *o = x;
            n += 1;
        } else {
            break;
...
fn wav_tab() -> &'static Mutex<Vec<Option<WavAcc>>> {
    WAV_TABLE.get_or_init(|| Mutex::new(Vec::new()))
}
...
pub fn wav_writer_write(h: i64, frames: &[f32]) -> Result<i64> {
    ...
    let acc = tab
        .get_mut(h as usize)
        .and_then(|o| o.as_mut())
        .ok_or_else(|| BasilError("Invalid WAV writer handle".into()))?;
...
pub fn wav_read_all(path: &str) -> Result<Vec<f32>> {
    ...
    #[cfg(not(feature = "obj-audio"))]
    {
        set_err("WAV read not available (hound disabled)");
        Err(BasilError("WAV read not available".into()))
    }
}

// --- Low-level Synth handle API (obj-audio) ---
static SYNTH_TABLE: OnceLock<Mutex<Vec<Option<Synth>>>> = OnceLock::new();
fn synth_tab() -> &'static Mutex<Vec<Option<Synth>>> {
    SYNTH_TABLE.get_or_init(|| Mutex::new(Vec::new()))
}
...
pub fn synth_note_on(h: i64, note: i64, vel: i64) -> Result<i64> {
    ...
    let s = tab
        .get_mut(h as usize)
        .and_then(|o| o.as_mut())
        .ok_or_else(|| BasilError("Invalid synth handle".into()))?;
...
pub fn synth_delete(h: i64) -> Result<i64> {
    ...
    if let Some(slot) = tab.get_mut(h as usize) {
        *slot = None;
        ...
        Err(BasilError("Invalid synth handle".into()))
    }
}
...
mod tests {
    ...
    fn wav_writer_roundtrip() {
        ...
        let rate = 48000.0f32;
        for i in 0..100 {
            buf[i] = (2.0 * std::f32::consts::PI * freq * (i as f32) / rate).sin() * 0.2;
        }
