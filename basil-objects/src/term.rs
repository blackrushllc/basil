...
fn state() -> &'static Mutex<TermState> {
    GLOBAL.get_or_init(|| Mutex::new(TermState::default()))
}
    ...
    match v {
        Value::Int(i) => {
            if *i == -1 {
                ...
                color_from_code(*i)
                    .ok_or_else(|| format!("invalid color code {}", i))
                    .map(Some)
            }
        }
        Value::Num(n) => {
        ...
        other => Err(format!(
            "color expects int 0..15, -1, or name; got {}",
            other
        )),
    }
}
...
pub fn cls() -> i64 {
    ...
    if let Some(fg) = s.fg {
        let _ = execute!(out, SetForegroundColor(fg));
    }
    if let Some(bg) = s.bg {
...
pub fn locate(x: &Value, y: &Value) -> i64 {
    ...
    let (cols, rows) = match terminal::size() {
        Ok(sz) => sz,
        Err(e) => {
            set_err(format!("terminal size: {}", e));
            (80, 25)
        }
    };
    ...
    match execute!(io::stdout(), cursor::MoveTo(xc - 1, yc - 1)) {
        Ok(_) => ok(),
        Err(e) => {
            set_err(format!("LOCATE failed: {}", e));
            1
        }
    }
}
...
pub fn color(fg: &Value, bg: &Value) -> i64 {
    ...
    let fgc = match parse_color_value(fg) {
        Ok(c) => c,
        Err(m) => {
            set_err(m);
            return 3;
    ...
    let bgc = match parse_color_value(bg) {
        Ok(c) => c,
        Err(m) => {
            set_err(m);
            return 3;
    ...
    if let Some(c) = fgc {
        st.fg = Some(c);
        if let Err(e) = execute!(out, SetForegroundColor(c)) {
            set_err(format!("set fg: {}", e));
            return 1;
        }
    }
    if let Some(c) = bgc {
        st.bg = Some(c);
        if let Err(e) = execute!(out, SetBackgroundColor(c)) {
            set_err(format!("set bg: {}", e));
            return 1;
        }
    }
    ok()
...
pub fn color_reset() -> i64 {
    ...
    st.fg = None;
    st.bg = None;
    if let Err(e) = execute!(io::stdout(), ResetColor) {
        set_err(format!("ResetColor: {}", e));
        return 1;
    }
    ok()
}
...
pub fn attr(bold: &Value, underline: &Value, reverse: &Value) -> i64 {
    ...
    let mut out = io::stdout();
    if bi == 0 {
        let _ = execute!(out, SetAttribute(Attribute::NoBold));
        st.bold = false;
    } else if bi == 1 {
    ...
    if ui == 0 {
        let _ = execute!(out, SetAttribute(Attribute::NoUnderline));
        st.underline = false;
    } else if ui == 1 {
        let _ = execute!(out, SetAttribute(Attribute::Underlined));
        st.underline = true;
    }
    if ri == 0 {
        let _ = execute!(out, SetAttribute(Attribute::NoReverse));
        st.reverse = false;
    } else if ri == 1 {
...
pub fn attr_reset() -> i64 {
    ...
    st.underline = false;
    st.reverse = false;
    if let Err(e) = execute!(io::stdout(), SetAttribute(Attribute::Reset)) {
        set_err(format!("ATTR_RESET: {}", e));
        return 1;
    }
    ok()
}
...
pub fn cursor_save() -> i64 {
    match cursor::position() {
        Ok((x, y)) => {
            let mut st = state().lock().unwrap();
            if st.pos_stack.len() >= 8 {
                st.pos_stack.remove(0);
            }
            st.pos_stack.push((x, y));
            ok()
        }
        Err(e) => {
            set_err(format!("CURSOR_SAVE: {}", e));
            1
        }
    }
}
...
pub fn cursor_restore() -> i64 {
    ...
    if let Some((x, y)) = st.pos_stack.pop() {
        match execute!(io::stdout(), cursor::MoveTo(x, y)) {
            Ok(_) => ok(),
            Err(e) => {
                set_err(format!("CURSOR_RESTORE: {}", e));
                1
            }
        }
    } else {
...
pub fn term_rows() -> i64 {
    terminal::size().map(|(_, r)| r as i64).unwrap_or(25)
}

pub fn cursor_hide() -> i64 {
    match execute!(io::stdout(), cursor::Hide) {
        Ok(_) => ok(),
        Err(e) => {
            set_err(format!("CURSOR_HIDE: {}", e));
            1
        }
    }
}
pub fn cursor_show() -> i64 {
    match execute!(io::stdout(), cursor::Show) {
        Ok(_) => ok(),
        Err(e) => {
            set_err(format!("CURSOR_SHOW: {}", e));
            1
        }
    }
}
...
pub fn term_raw(mode: &Value) -> i64 {
    ...
    let on = match mode {
        ...
        _ => {
            set_err(format!("TERM.RAW expects ON/OFF or 0/1, got {}", mode));
            return 2;
        }
    };
    if !stdout_is_tty() {
    ...
    if on {
        if !st.raw_on {
            if let Err(e) = terminal::enable_raw_mode() {
                set_err(format!("enable_raw_mode: {}", e));
                return 1;
            }
            st.raw_on = true;
        }
        ...
        if st.raw_on {
            if let Err(e) = terminal::disable_raw_mode() {
                set_err(format!("disable_raw_mode: {}", e));
                return 1;
            }
            st.raw_on = false;
        }
...
pub fn altscreen_off() -> i64 {
    ...
    if st.alt_on {
        if let Err(e) = execute!(io::stdout(), LeaveAlternateScreen) {
            set_err(format!("ALTSCREEN_OFF: {}", e));
            return 1;
        }
        st.alt_on = false;
    }
