...
fn main() {
    ...
    if cmd != "aot" {
        eprintln!("error: unknown command '{}'. Use 'bcc aot <file>'.", cmd);
        std::process::exit(2);
    }
    if args.is_empty() {
        eprintln!("error: missing <input.basil>");
        std::process::exit(2);
    }
    ...
    while i < args.len() {
        match args[i].as_str() {
            "-o" => {
                i += 1; /* planned: output dir for exe */
                i += 1;
            }
            "--name" => {
                i += 1;
            ...
            "--features" => {
                i += 1;
                features_spec = args.get(i).cloned();
                i += 1;
            }
            "--target" => {
                i += 1;
            ...
            "--opt" => {
                i += 1;
                opt_level = args.get(i).and_then(|s| s.parse::<u8>().ok());
                i += 1;
            }
            "--lto" => {
                i += 1;
                lto = args.get(i).cloned();
                i += 1;
            }
            "--emit-project" => {
                i += 1;
                emit_project_dir = args.get(i).map(|s| PathBuf::from(s));
                i += 1;
            }
            "--dep-source" => {
                i += 1;
                dep_source_choice = args.get(i).cloned();
                i += 1;
            }
            "--local-runtime" => {
                i += 1;
                local_runtime_root = args.get(i).map(|s| PathBuf::from(s));
                i += 1;
            }
            "--vendor-dir" => {
                i += 1;
                vendor_dir = args.get(i).map(|s| PathBuf::from(s));
                i += 1;
            }
            "--keep-build" => {
                keep_build = true;
    ...
    let (rt_features, obj_crates) = match features_spec.as_deref() {
        ...
        Some(list) => {
            ...
            for o in objs {
                if let Some((r, oc)) = map_obj_to_rt(o) {
                    rt.push(r);
                    objc.push(oc);
                }
            }
    ...
    let dep_source = match dep_source_choice.as_deref() {
        ...
        Some("crates-io") | None => {
            if dep_source_choice.is_none() {
                ...
                if let Some(vd) = vendor_dir.clone() {
                    DepSource::Vendor(vd)
                } else if let Some(root) = local_runtime_root.clone() {
                    DepSource::LocalPath(root)
                } else {
                    DepSource::CratesIo
                }
        ...
        Some(other) => {
            eprintln!(
                "warning: unknown --dep-source '{}', defaulting to crates-io",
                other
            );
            DepSource::CratesIo
        }
    ...
    let emitted = match emit_project(&base_dir, &input_path, &module, &opts) {
        Ok(p) => p,
        Err(e) => {
            eprintln!("error: failed to emit project: {}", e);
            std::process::exit(1);
        }
    };
    ...
    if emit_project_dir.is_some() {
        if !quiet {
            println!("project written to {}", emitted.root.display());
        }
        return;
    }
    ...
    if !quiet {
        println!("building with Cargo in {}", emitted.root.display());
    }
    ...
    if !quiet {
        println!("ok: project at {}", emitted.root.display());
    }
    // Determine built executable path inside the emitted project
    let bin_name = opts
        .name
        .clone()
        .unwrap_or_else(|| "basil_prog".to_string());
    let bin_dir = if let Some(tgt) = &opts.target {
        emitted.root.join("target").join(tgt).join("release")
    } else {
        emitted.root.join("target").join("release")
    };
    let built_exe = if cfg!(windows) {
        bin_dir.join(format!("{}.exe", bin_name))
    } else {
    ...
    if !moved_ok {
        match fs::copy(&built_exe, &out_path) {
            Ok(_) => {
                let _ = fs::remove_file(&built_exe);
            }
            Err(e) => {
                eprintln!(
                    "error: failed to write executable to {}: {}",
                    out_path.display(),
                    e
                );
                std::process::exit(1);
...
fn first_missing_required_feature(src: &str, rt_features: &Vec<String>) -> Option<MissingFeature> {
    ...
    if src.contains("AUDIO_") && !has("audio") {
        return Some(MissingFeature {
            hint: "AUDIO_* symbol".into(),
            required_obj: "obj-audio".into(),
            suggest_use: "AUDIO",
            cli_name: "audio",
        });
    }
    if src.contains("MIDI_") && !has("midi") {
        return Some(MissingFeature {
            hint: "MIDI_* symbol".into(),
            required_obj: "obj-midi".into(),
            suggest_use: "MIDI",
            cli_name: "midi",
        });
    }
    if src.contains("DAW_") && !has("daw") {
        return Some(MissingFeature {
            hint: "DAW_* symbol".into(),
            required_obj: "obj-daw".into(),
            suggest_use: "DAW",
            cli_name: "daw",
        });
    }
    if src.contains("TERM_") && !has("term") {
        return Some(MissingFeature {
            hint: "TERM_* symbol".into(),
            required_obj: "obj-term".into(),
            suggest_use: "TERM",
            cli_name: "term",
        });
    }
...
fn autodetect_features(src: &str) -> Vec<String> {
    ...
    for line in src.lines() {
        ...
        if t.starts_with("#USE ") {
            ...
            if uses.contains("audio") {
                v.push("audio".into());
            }
            if uses.contains("midi") {
                v.push("midi".into());
            }
            if uses.contains("daw") {
                v.push("daw".into());
            }
            if uses.contains("term") {
                v.push("term".into());
            }
        }
    }
    if src.contains("AUDIO_") {
        v.push("audio".into());
    }
    if src.contains("MIDI_") {
        v.push("midi".into());
    }
    if src.contains("DAW_") {
        v.push("daw".into());
    }
    if src.contains("TERM_") {
        v.push("term".into());
    }
    // Dedup
