...
fn err_msg(op: &str, e: impl std::fmt::Display) -> BasilError {
    BasilError(format!("SQS.{}: {}", op, e))
}

pub struct SqsObject {
...
impl BasicObject for SqsObject {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("AWS_SQS has no readable properties".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("AWS_SQS has no settable properties".into()))
    }
        ...
        match method.to_ascii_uppercase().as_str() {
            "SEND$" => {
                if !(2..=3).contains(&args.len()) {
                    return Err(BasilError(
                        "SQS.Send$ expects queue_url$, body$, delay_sec%?".into(),
                    ));
                }
                let queue_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                };
            ...
            "RECEIVE$" => {
                if !(1..=4).contains(&args.len()) {
                    return Err(BasilError(
                        "SQS.Receive$ expects queue_url$, max%?, wait_sec%?, vis_timeout_sec%?"
                            .into(),
                    ));
                }
                let queue_url = match &args[0] {
            ...
            "DELETE$" => {
                if args.len() != 2 {
                    return Err(BasilError(
                        "SQS.Delete$ expects queue_url$, receipt_handle$".into(),
                    ));
                }
                let queue_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                RT.block_on(async move {
                    let _ = client
                        ...
                        .send()
                        .await
                        .map_err(|e| err_msg("Delete", e))?;
                    Ok::<Value, BasilError>(Value::Int(1))
                })
            }
            "PURGE" => {
                if args.len() != 1 {
                    return Err(BasilError("SQS.Purge expects queue_url$".into()));
                }
                let queue_url = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
    ...
    ObjectDescriptor {
        ...
        methods: vec![
            MethodDesc {
                name: "Send$".into(),
                arity: 3,
                arg_names: vec!["queue_url$".into(), "body$".into(), "delay_sec%?".into()],
            ...
            MethodDesc {
                name: "Delete$".into(),
                arity: 2,
                arg_names: vec!["queue_url$".into(), "receipt_handle$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "Purge".into(),
                arity: 1,
                arg_names: vec!["queue_url$".into()],
                return_type: "Int (ok%)".into(),
            },
        ],
        examples: vec!["DIM aws@ AS AWS() : DIM sqs@ = aws@.MakeSQS()".into()],
    }
}
