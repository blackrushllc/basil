...
impl BasicObject for S3Object {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("AWS_S3 has no readable properties".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("AWS_S3 has no settable properties".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "PUT$" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "S3.Put$ expects 3 args: bucket$, key$, data$|file$".into(),
                    ));
                }
                ...
                let etag = if std::path::Path::new(&third).exists() {
                    ...
                    f.read_to_end(&mut buf)
                        .map_err(|e| err_msg("Put(read file)", e))?;
            ...
            "GET$" => {
                if args.len() != 2 {
                    return Err(BasilError("S3.Get$ expects 2 args: bucket$, key$".into()));
                }
                let bucket = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                ...
                let data = RT.block_on(async move {
                    let resp = client
                        ...
                        .send()
                        .await
                        .map_err(|e| err_msg("Get", e))?;
                    let bytes = resp
                        .body
                        .collect()
                        .await
                        .map_err(|e| err_msg("Get(read)", e))?
                        .into_bytes();
            ...
            "GETTOFILE" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "S3.GetToFile expects 3 args: bucket$, key$, file$".into(),
                    ));
                }
                let bucket = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let data = RT.block_on(async move {
                    let resp = client
                        ...
                        .send()
                        .await
                        .map_err(|e| err_msg("GetToFile", e))?;
                    let bytes = resp
                        .body
                        .collect()
                        .await
                        .map_err(|e| err_msg("GetToFile(read)", e))?
                        .into_bytes();
                    Ok::<Vec<u8>, BasilError>(bytes.to_vec())
                })?;
                let mut f = File::create(&file).map_err(|e| err_msg("GetToFile(create)", e))?;
                f.write_all(&data)
                    .map_err(|e| err_msg("GetToFile(write)", e))?;
                Ok(Value::Int(1))
            }
            "LIST$" => {
                if !(args.len() == 1 || args.len() == 2 || args.len() == 3) {
                    return Err(BasilError(
                        "S3.List$ expects bucket$, prefix$?, max%?".into(),
                    ));
                }
                let bucket = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let keys: Vec<String> = RT.block_on(async move {
                    ...
                    loop {
                        ...
                        if resp.is_truncated.unwrap_or(false) && next.is_some() {
                            token = next;
                        } else {
                            break;
                        }
            ...
            "DELETE" => {
                if args.len() != 2 {
                    return Err(BasilError("S3.Delete expects 2 args: bucket$, key$".into()));
                }
                let bucket = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
            ...
            "SIGNEDURL$" => {
                if args.len() != 3 {
                    return Err(BasilError(
                        "S3.SignedUrl$ expects 3 args: bucket$, key$, expires_sec%".into(),
                    ));
                }
                let bucket = match &args[0] {
                    Value::Str(s) => s.clone(),
                ...
                let url = RT.block_on(async move {
                    ...
                    let pres =
                        PresigningConfig::expires_in(std::time::Duration::from_secs(expires))
                            .map_err(|e| err_msg("SignedUrl(config)", e))?;
                    let req = client.get_object().bucket(bucket).key(key);
                    let presigned = req
                        .presigned(pres)
                        .await
                        .map_err(|e| err_msg("SignedUrl(presign)", e))?;
    ...
    ObjectDescriptor {
        ...
        methods: vec![
            MethodDesc {
                name: "Put$".into(),
                arity: 3,
                arg_names: vec!["bucket$".into(), "key$".into(), "data$|file$".into()],
                return_type: "String (ETag$)".into(),
            },
            MethodDesc {
                name: "Get$".into(),
                arity: 2,
                arg_names: vec!["bucket$".into(), "key$".into()],
                return_type: "String (bytes$)".into(),
            },
            MethodDesc {
                name: "GetToFile".into(),
                arity: 3,
                arg_names: vec!["bucket$".into(), "key$".into(), "file$".into()],
                return_type: "Int (ok%)".into(),
            },
            MethodDesc {
                name: "List$".into(),
                arity: 3,
                arg_names: vec!["bucket$".into(), "prefix$?".into(), "max%?".into()],
            ...
            MethodDesc {
                name: "SignedUrl$".into(),
                arity: 3,
                arg_names: vec!["bucket$".into(), "key$".into(), "expires_sec%".into()],
                return_type: "String (url$)".into(),
            },
        ],
        examples: vec!["DIM aws@ AS AWS() : DIM s3@ = aws@.MakeS3()".into()],
    }
}
...
pub fn register<F: FnMut(&str, crate::TypeInfo)>(reg: &mut F) {
    let factory = |_args: &[Value]| -> Result<Rc<RefCell<dyn BasicObject>>> {
        ...
        match val {
            Value::Object(o) => Ok(o),
            _ => Err(BasilError("unexpected".into())),
        }
    };
