...
fn err_msg(op: &str, e: impl std::fmt::Display) -> BasilError {
    BasilError(format!("SES.{}: {}", op, e))
}

pub struct SesObject {
    pub client: aws_sdk_sesv2::Client,
}
impl SesObject {
    pub fn from_client(client: aws_sdk_sesv2::Client) -> Self {
        Self { client }
    }
}
...
impl BasicObject for SesObject {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("AWS_SES has no readable properties".into()))
    }
    fn set_prop(&mut self, _name: &str, _v: Value) -> Result<()> {
        Err(BasilError("AWS_SES has no settable properties".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "SENDEMAIL" => {
                if !(3..=6).contains(&args.len()) {
                    return Err(BasilError(
                        "SES.SendEmail expects to$, subject$, body$, from$?, reply_to$?, is_html%?"
                            .into(),
                    ));
                }
                let to = match &args[0] {
                ...
                if from.is_empty() {
                    return Err(BasilError("SES.SendEmail: from$ is required".into()));
                }
                ...
                let msg_id = RT.block_on(async move {
                    ...
                    let dest = Destination::builder()
                        .set_to_addresses(Some(to_addrs))
                        .build();
                    let subject_c = Content::builder()
                        .data(subject)
                        .build()
                        .map_err(|e| err_msg("SendEmail(subject)", e))?;
                    let mut b = Body::builder();
                    if is_html {
                        let html_c = Content::builder()
                            .data(body.clone())
                            .build()
                            .map_err(|e| err_msg("SendEmail(html)", e))?;
                        b = b.html(html_c);
                    } else {
                        let text_c = Content::builder()
                            .data(body.clone())
                            .build()
                            .map_err(|e| err_msg("SendEmail(text)", e))?;
                        b = b.text(text_c);
                    }
                    ...
                    if let Some(rt) = reply_to {
                        req = req.set_reply_to_addresses(Some(vec![rt]));
                    }
            ...
            "SENDRAW$" => {
                if args.len() != 1 {
                    return Err(BasilError("SES.SendRaw$ expects mime$".into()));
                }
                let mime = match &args[0] {
                    Value::Str(s) => s.clone(),
                    other => format!("{}", other),
                };
                let client = self.client.clone();
                let msg_id = RT.block_on(async move {
                    ...
                    let raw = RawMessage::builder()
                        .data(bytes.into())
                        .build()
                        .map_err(|e| err_msg("SendRaw(raw)", e))?;
    ...
    ObjectDescriptor {
        ...
        methods: vec![
            MethodDesc {
                ...
                return_type: "String (message_id$)".into(),
            },
            MethodDesc {
                name: "SendRaw$".into(),
                arity: 1,
                arg_names: vec!["mime$".into()],
                return_type: "String (message_id$)".into(),
            },
        ],
        examples: vec!["DIM aws@ AS AWS() : DIM ses@ = aws@.MakeSES()".into()],
    }
}
