...
fn bad_arity(name: &str, expect: usize, got: usize) -> BasilError {
    BasilError(format!(
        "DB_MYSQL.{}: expected {} args, got {}",
        name, expect, got
    ))
}

fn err(op: &str, e: impl std::fmt::Display) -> BasilError {
    BasilError(format!("SQL(MySQL) {}: {}", op, e))
}
...
impl MySqlObj {
    ...
    fn ensure_pool(&mut self) -> Result<sqlx::MySqlPool> {
        ...
        let dsn = self.dsn.clone().ok_or_else(|| {
            BasilError("DB_MYSQL: no DSN configured; pass in constructor or call Connect$".into())
        })?;

        // Build options from DSN
        let mut opts: sqlx::mysql::MySqlConnectOptions = dsn
            .parse()
            .map_err(|_e| BasilError("SQL(MySQL) ConnectFailed: bad DSN".into()))?;
        // TLS mode mapping
    ...
    fn bind_mysql<'q>(
        mut qb: sqlx::query::Query<'q, sqlx::MySql, sqlx::mysql::MySqlArguments>,
        params: &'q [String],
        ...
        for row in rows.iter() {
            ...
            for col in row.columns() {
                ...
                let jv = if let Ok(v) = row.try_get::<i64, _>(name.as_str()) {
                    J::from(v)
                } else if let Ok(v) = row.try_get::<u64, _>(name.as_str()) {
                    J::from(v)
...
impl BasicObject for MySqlObj {
    ...
    fn set_prop(&mut self, name: &str, v: Value) -> Result<()> {
        match name.to_ascii_uppercase().as_str() {
            ...
            "TLSMODE$" => {
                self.tls_mode = str_arg(&v).to_ascii_uppercase();
                self.pool = None;
                Ok(())
            }
            "ROOTCERTPATH$" => {
        ...
        match method.to_ascii_uppercase().as_str() {
            "CONNECT$" => {
                ...
                let dsn = format!(
                    ...
                    port,
                    db,
                    if ssl.is_empty() { "REQUIRED" } else { &ssl }
                );
                self.dsn = Some(dsn);
                self.pool = None;
                // try connect now
            ...
            "BEGIN" => {
                ...
                // BEGIN (MySQL: use START TRANSACTION via text protocol to avoid prepared-statement limitation)
                TOKIO_RT
                    .block_on(async { conn.execute("START TRANSACTION").await })
            ...
            "COMMIT" => {
                if let Some(mut conn) = self.txn_conn.take() {
                    let res = TOKIO_RT.block_on(async move {
                        let r = conn.execute("COMMIT").await;
            ...
            "ROLLBACK" => {
                if let Some(mut conn) = self.txn_conn.take() {
                    let res = TOKIO_RT.block_on(async move {
                        let r = conn.execute("ROLLBACK").await;
                        // Drop the connection inside the Tokio context to avoid runtime panic
                        drop(conn);
                        r
                    });
