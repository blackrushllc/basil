use nettle_sys::{
    aes128_ctx, nettle_aes128_decrypt, nettle_aes128_encrypt,
    nettle_aes128_invert_key, nettle_aes128_set_decrypt_key,
...
pub struct Aes128 {
    context: Box<aes128_ctx>,
}

impl_zeroing_drop_for!(Aes128);

impl Aes128 {
    /// Creates a new `Aes128` instance for decryption that uses the same key as `encrypt`.
    ///
    ...
    pub fn with_inverted_key(encrypt: &Self) -> Self {
        let context = unsafe {
            ...
            nettle_aes128_invert_key(ctx.as_mut_ptr(),
...
impl Cipher for Aes128 {
    ...
    fn with_decrypt_key(key: &[u8]) -> Result<Aes128> {
        ...
        let context = unsafe {
            let mut ctx = Box::new(MaybeUninit::uninit());
            nettle_aes128_set_decrypt_key(ctx.as_mut_ptr(), key.as_ptr());
            transmute(ctx)
        };
    ...
    fn context(&mut self) -> *mut c_void {
        (self.context.as_mut() as *mut aes128_ctx) as *mut c_void
    }
