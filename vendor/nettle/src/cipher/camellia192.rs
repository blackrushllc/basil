use nettle_sys::{
    camellia256_ctx, nettle_camellia192_set_decrypt_key,
    nettle_camellia192_set_encrypt_key, nettle_camellia256_crypt,
...
impl_zeroing_drop_for!(Camellia192);

impl Camellia192 {
    /// Creates a new `Camellia192` instance for decryption that uses the same key as `encrypt`.
    ///
...
impl Cipher for Camellia192 {
    ...
    fn with_encrypt_key(key: &[u8]) -> Result<Camellia192> {
        ...
        let context = unsafe {
            let mut ctx = Box::new(MaybeUninit::uninit());
            nettle_camellia192_set_encrypt_key(ctx.as_mut_ptr(), key.as_ptr());
            transmute(ctx)
        };

        Ok(Camellia192 { context })
    }
    ...
    fn with_decrypt_key(key: &[u8]) -> Result<Camellia192> {
        ...
        let context = unsafe {
            let mut ctx = Box::new(MaybeUninit::uninit());
            nettle_camellia192_set_decrypt_key(ctx.as_mut_ptr(), key.as_ptr());
            transmute(ctx)
        };
