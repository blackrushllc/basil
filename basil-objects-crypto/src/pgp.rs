...
use basil_bytecode::{BasicObject, MethodDesc, ObjectDescriptor, ObjectRef, Value};
...
use openpgp::serialize::stream::{
    Armorer, Encryptor2 as Encryptor, LiteralWriter, Message, Signer,
};
...
fn descriptor_static() -> ObjectDescriptor {
    ObjectDescriptor {
        ...
        methods: vec![
            MethodDesc {
                name: "EncryptArmored$".to_string(),
                arity: 2,
                arg_names: vec!["public_key_armored$".into(), "plaintext$".into()],
                return_type: "String".into(),
            },
            MethodDesc {
                name: "DecryptArmored$".to_string(),
                arity: 3,
                arg_names: vec![
                    "private_key_armored$".into(),
                    "passphrase$".into(),
                    "cipher_armored$".into(),
                ],
                return_type: "String".into(),
            ...
            MethodDesc {
                ...
                arity: 3,
                arg_names: vec![
                    "public_key_armored$".into(),
                    "message$".into(),
                    "signature_armored$".into(),
                ],
                return_type: "Int".into(),
            },
            MethodDesc {
                name: "EncryptFile$".to_string(),
                arity: 3,
                arg_names: vec![
                    "public_key_armored$".into(),
                    "in_path$".into(),
            ...
            MethodDesc {
                name: "ReadFileText$".to_string(),
                arity: 1,
                arg_names: vec!["path$".into()],
                return_type: "String".into(),
            },
            MethodDesc {
                name: "WriteFileText$".to_string(),
                arity: 2,
                arg_names: vec!["path$".into(), "text$".into()],
                return_type: "Int".into(),
            },
        ],
        examples: vec![
...
impl BasicObject for PgpObj {
    ...
    fn get_prop(&self, _name: &str) -> Result<Value> {
        Err(BasilError("CRYPTO_PGP has no properties".into()))
    }
    ...
    fn call(&mut self, method: &str, args: &[Value]) -> Result<Value> {
        match method.to_ascii_uppercase().as_str() {
            "ENCRYPTARMORED$" => {
                if args.len() != 2 {
                    return Err(bad_arity("EncryptArmored$", 2, args.len()));
                }
            ...
            "DECRYPTARMORED$" => {
                if args.len() != 3 {
                    return Err(bad_arity("DecryptArmored$", 3, args.len()));
                }
            ...
            "SIGNARMORED$" => {
                if args.len() != 3 {
                    return Err(bad_arity("SignArmored$", 3, args.len()));
                }
                let sec_armored = str_arg(&args[0]);
                let pass = str_arg(&args[1]);
                let msg = str_arg(&args[2]);
                let sig =
                    sign_detached_armored(&sec_armored, empty_to_none(&pass), msg.as_bytes())?;
                Ok(Value::Str(sig))
            }
            "VERIFY$" => {
                if args.len() != 3 {
                    return Err(bad_arity("Verify$", 3, args.len()));
                }
                let pub_armored = str_arg(&args[0]);
            ...
            "ENCRYPTFILE$" => {
                if args.len() != 3 {
                    return Err(bad_arity("EncryptFile$", 3, args.len()));
                }
                let pub_armored = str_arg(&args[0]);
                ...
                File::open(&in_path)
                    .map_err(|e| BasilError(format!("PGP.EncryptFile: {}", e)))?
                    .read_to_end(&mut input)
                    .map_err(|e| BasilError(format!("PGP.EncryptFile: {}", e)))?;
                let cipher = encrypt_armored(&pub_armored, &input)?;
                std::fs::write(&out_path, cipher)
                    .map_err(|e| BasilError(format!("PGP.EncryptFile: {}", e)))?;
                Ok(Value::Int(1))
            }
            "DECRYPTFILE$" => {
                if args.len() != 4 {
                    return Err(bad_arity("DecryptFile$", 4, args.len()));
                }
                let sec_armored = str_arg(&args[0]);
                let pass = str_arg(&args[1]);
                let in_path = str_arg(&args[2]);
                let out_path = str_arg(&args[3]);
                let cipher = std::fs::read(&in_path)
                    .map_err(|e| BasilError(format!("PGP.DecryptFile: {}", e)))?;
            ...
            "SIGNFILE$" => {
                if args.len() != 4 {
                    return Err(bad_arity("SignFile$", 4, args.len()));
                }
                let sec_armored = str_arg(&args[0]);
                let pass = str_arg(&args[1]);
                let in_path = str_arg(&args[2]);
                ...
                let mut input = Vec::new();
                File::open(&in_path)
                    .map_err(|e| BasilError(format!("PGP.SignFile: {}", e)))?
                    .read_to_end(&mut input)
                    .map_err(|e| BasilError(format!("PGP.SignFile: {}", e)))?;
                let sig = sign_detached_armored(&sec_armored, empty_to_none(&pass), &input)?;
                std::fs::write(&sig_out, sig)
                    .map_err(|e| BasilError(format!("PGP.SignFile: {}", e)))?;
                Ok(Value::Int(1))
            }
            "VERIFYFILE$" => {
                if args.len() != 3 {
                    return Err(bad_arity("VerifyFile$", 3, args.len()));
                }
                let pub_armored = str_arg(&args[0]);
                let in_path = str_arg(&args[1]);
                let sig_path = str_arg(&args[2]);
                let data = std::fs::read(&in_path)
                    .map_err(|e| BasilError(format!("PGP.VerifyFile: {}", e)))?;
            ...
            "READFILETEXT$" => {
                if args.len() != 1 {
                    return Err(bad_arity("ReadFileText$", 1, args.len()));
                }
                let p = str_arg(&args[0]);
                let s = std::fs::read_to_string(&p)
                    .map_err(|e| BasilError(format!("PGP.ReadFileText: {}", e)))?;
                Ok(Value::Str(s))
            }
            "WRITEFILETEXT$" => {
                if args.len() != 2 {
                    return Err(bad_arity("WriteFileText$", 2, args.len()));
                }
                let p = str_arg(&args[0]);
                let s = str_arg(&args[1]);
                std::fs::write(&p, s)
                    .map_err(|e| BasilError(format!("PGP.WriteFileText: {}", e)))?;
                Ok(Value::Int(1))
            }
            other => Err(BasilError(format!(
                "Unknown method '{}' on CRYPTO_PGP",
                other
            ))),
        }
    }
...
fn bad_arity(name: &str, want: usize, got: usize) -> BasilError {
    BasilError(format!("{} expects {} arguments (got {})", name, want, got))
}
...
fn encrypt_armored(recipient_pubkey_armored: &str, data: &[u8]) -> Result<String> {
    ...
    if recipients.is_empty() {
        return Err(BasilError(
            "PGP.Encrypt: No valid encryption key found".into(),
        ));
    }
    ...
    let msg = Armorer::new(msg)
        ...
        .build()
        .map_err(|e| BasilError(format!("PGP.Encrypt: {}", e)))?;
    let enc = Encryptor::for_recipients(msg, recipients.iter().map(|ka| ka.key()))
        .build()
        .map_err(|e| BasilError(format!("PGP.Encrypt: {}", e)))?;
    let mut lit = LiteralWriter::new(enc)
        .build()
        .map_err(|e| BasilError(format!("PGP.Encrypt: {}", e)))?;
    lit.write_all(data)
        .map_err(|e| BasilError(format!("PGP.Encrypt: {}", e)))?;
    lit.finalize()
        .map_err(|e| BasilError(format!("PGP.Encrypt: {}", e)))?;
    let armored =
        String::from_utf8(out).map_err(|_| BasilError("PGP.Encrypt: Output not UTF-8".into()))?;
    Ok(armored)
}
...
impl DecryptionHelper for HelperDec {
    fn decrypt<D>(
        ...
        mut decrypt: D,
    ) -> AnyhowResult<Option<openpgp::Fingerprint>>
    where
...
impl VerificationHelper for HelperDec {
    ...
    fn check(&mut self, _structure: MessageStructure<'_>) -> AnyhowResult<()> {
        Ok(())
    }
}
...
fn sign_detached_armored(
    ...
    let keypair =
        maybe_keypair.ok_or_else(|| BasilError("PGP.Sign: No usable signing key".into()))?;
    ...
    let msg = Armorer::new(msg)
        ...
        .build()
        .map_err(|e| BasilError(format!("PGP.Sign: {}", e)))?;
    let mut ds = Signer::new(msg, keypair)
        .detached()
        .build()
        .map_err(|e| BasilError(format!("PGP.Sign: {}", e)))?;
    ds.write_all(data)
    ...
    ds.finalize()
        .map_err(|e| BasilError(format!("PGP.Sign: {}", e)))?;
    let sig =
        String::from_utf8(out).map_err(|_| BasilError("PGP.Sign: Output not UTF-8".into()))?;
    Ok(sig)
}
